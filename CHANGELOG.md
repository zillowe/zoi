...
- Update binary installation paths to use standard local bin directories
- *(cli)* Consolidate clean command into cache clear

### 🩹 Bug Fixes

- *(package)* Relocate ELFs before pooling to ensure manifest consistency
...
- Branch 'renovate/clap_complete-4.x' into 'main'
- Branch 'renovate/base64-0.x' into 'main'
- Branch 'renovate/serde-monorepo' into 'main'
- Branch 'renovate/diffy-0.x' into 'main'
- Branch 'renovate/clap-4.x' into 'main'
- Branch 'renovate/anyhow-1.x' into 'main'
- Branch 'renovate/ignore-0.x' into 'main'
- Branch 'renovate/glob-0.x' into 'main'
- Branch 'renovate/serde_json-1.x' into 'main'
- Branch 'renovate/thiserror-2.x' into 'main'
...
- *(pkg)* Replace remote manual spec with zman helper
- *(lua)* Add UTILS.DOWNLOAD utility with hash verification
- *(api)* Implement persistent shell sessions for cmd()
- Implement package epochs and test dependency support
- *(lua)* Support .zpa and .zsa archive formats
- *(package)* Add pure build mode with isolated sysroot
- *(system)* Implement bootstrap diagnostics and improve chroot isolation
- *(pkg)* Improve backup file handling and add progress tracking
- *(daemon)* Integrate zoid daemon into build and packaging
- *(sandbox)* Implement sysroot execution via bubblewrap
- *(system)* Transition to transaction-based OS management

### 🏗️ Structure

- Reorganize core path and home directory logic

### 🔧 Configuration

- *(build)* Add shebang to Justfile

### 🛡️ Dependencies

- Update Rust crate clap_complete to 4.6.8
- Update Rust crate base64 to 0.23.0
- Update Rust crate serde to 1.0.229
- Update Rust crate diffy to 0.5.1
- Update Rust crate clap to 4.6.4
- Update Rust crate anyhow to 1.0.104
- Update Rust crate clap to 4.6.3
- Update Rust crate ignore to 0.4.31
- Update Rust crate glob to 0.3.4
- Update Rust crate serde_json to 1.0.151
- Update Rust crate thiserror to 2.0.19

### 🧹 Cleanup

- *(distro)* Simplify system diagnostics and improve error reporting
