...
pub fn generate_metadata(registry_root: &Path) -> Result<()> {
        ...
        .filter_map(|e| e.ok())
    {
        if entry.file_type().is_file() && entry.file_name().to_string_lossy().ends_with(".pkg.lua")
        {
            ...
            if let Ok(pkg) = crate::pkg::lua::parser::parse_lua_package(&path_str, None, true) {
                ...
                if let Ok(sec_entries) = fs::read_dir(pkg_dir) {
                    for sec_entry in sec_entries.flatten() {
                            ...
                            .ends_with(".sec.yaml")
                        {
                            ...
                            {
                                vulns.push(crate::pkg::mini_resolve::MiniVulnerability {
                                    id: adv.id,
                ...
                packages_map.insert(
                    pkg.name.clone(),
                    crate::pkg::purl::PurlPackageIndex {
                        repo: repo_path,
                        repo_type,
                        version,
    ...
    let index = crate::pkg::purl::RegistryIndex {
        version: "1".to_string(),
...
pub fn check(registry_root: &Path) -> Result<()> {
        ...
        .filter_map(|e| e.ok())
    {
        if entry.file_type().is_file() && entry.file_name().to_string_lossy().ends_with(".pkg.lua")
        {
            ...
            match crate::pkg::package::doctor::run(entry.path(), None, None) {
                Ok(report) => {
