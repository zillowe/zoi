...
pub fn ensure_archive_is_cached(node: &InstallNode, m: Option<&MultiProgress>) -> Result<PathBuf> {
    let pkg = &node.pkg;
    ...
    if let Ok(repo_config) = config::read_repo_config(&repo_db_path) {
        ...
        for pkg_link in pkg_links_to_try {
            ...
            return Ok(cached_archive_path);
        }
    }

    Err(anyhow!("No pre-built package found for '{}'", pkg.name))
}
...
fn run_install_flow(
    ...
    if mode == InstallMode::PreferPrebuilt
        && let Ok(cached_archive_path) = ensure_archive_is_cached(node, m)
    {
        ...
        if let Ok(installed_files) = crate::pkg::package::install::run(
            ...
            println!("Failed to install from archive. Building from source...");
        }
    }
