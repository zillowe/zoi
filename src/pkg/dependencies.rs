    ...
    match manager {
        "zoi" => {
            ...
            if let Ok(resolved_source) = resolve::resolve_source(&zoi_dep_name)
                && let Ok(content) = fs::read_to_string(&resolved_source.path)
                && let Ok(dep_pkg) = serde_yaml::from_str::<types::Package>(&content)
                && (dep_pkg.conflicts.is_some() || dep_pkg.bins.is_some())
            {
