...
/// - If it is invoked via a symlink (e.g. `node` -> `zoi`), it acts as a
///   "Shim".
...
fn main() {
        ...
        && !program_name.contains("target",)
    {
        ...
        //
        // This is a unique Zoi feature: if a shim resolves to a version that is
        // NOT currently installed (e.g. via a project's .tool-versions
        // file), Zoi doesn't just fail. Instead, it triggers this
        // callback to:
