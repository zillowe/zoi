    ...
    match cli.command {
        ...
        Commands::Stop => {
            ...
            match send_request(Request::Shutdown,) {
                ...
                Err(e,) => {
                    eprintln!(
                        "Failed to connect to daemon: {}. Checking for PID \
                         file...",
                        e
                    );
...
fn handle_client(
    ...
    let response = match request {
        ...
        Request::RollbackGeneration(target_id,) => {
            ...
            if target_id >= current_id {
                Response::Error(format!(
                    "Target generation {} is not older than current \
                     generation {}",
        ...
        Request::ApplySystemConfig(config,) => {
            ...
            // configuration.
            if let Ok(installed,) =
                zoi_resolver::local::get_installed_packages()
            {
                ...
                match gen_manager.create_generation_with_transaction(
                    ...
                    Ok(id,) => {
                        ...
                        if let Ok(gens,) = gen_manager.list_generations()
                            ...
                            && let Ok(bootloader,) =
                                zoi_system::boot::detect_bootloader()
                            && let Ok((kernel, initrd,),) =
                                gen_manager.find_boot_assets(new_gen,)
                        {
                            let cmdline = config
                                .system
                                .kernel_params
                                .as_deref()
                                .unwrap_or("",);
                            if let Err(e,) = bootloader.install_entry(
                                new_gen, &kernel, &initrd, cmdline,
                            ) {
                                boot_msg = format!(
                        ...
                        if let Err(e,) = gen_manager.activate_generation(id,) {
                            ...
                            {
                                eprintln!(
                                    "Warning: Failed to update /etc/fstab: {}",
                                    e
                                );
                            }
