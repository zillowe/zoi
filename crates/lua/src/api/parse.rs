...
/// - `checksumFile`: A specialized parser for standard checksum files (e.g.
///   `sha256sums`).
///
...
pub fn add_parse_util(lua: &Lua,) -> Result<(), mlua::Error,> {
    ...
    let checksum_fn =
        lua.create_function(|_, (content, file_name,): (String, String,)| {
            for line in content.lines() {
                let parts: Vec<&str,> = line.split_whitespace().collect();
                if parts.len() == 2
                    && parts.get(1,) == Some(&file_name.as_str(),)
                {
