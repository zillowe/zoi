                ...
                while let Some(entry_result) = ar.next_entry() {
                    ...
                    if name.starts_with("data.tar") {
                        ...
                        let mut temp_file = fs::File::create(&temp_data_path)
                            .map_err(|e| {
                            mlua::Error::RuntimeError(format!(
                                "Failed to create temp file for {name}: {e}"
                            ))
                        })?;
                        ...
                        {
                            let mut archive =
                                tar::Archive::new(GzDecoder::new(data_file));
                        ...
                        {
                            let mut archive =
                                tar::Archive::new(XzDecoder::new(data_file));
                            archive.unpack(&out_dir).map_err(|e| {
                                mlua::Error::RuntimeError(format!(
                                    "Failed to unpack {name}: {e}"
                                ))
                            ...
                            .is_some_and(|ext| ext.eq_ignore_ascii_case("zst"))
                        {
                            let mut archive = tar::Archive::new(
                                ZstdDecoder::new(data_file).map_err(|e| {
                                    mlua::Error::RuntimeError(format!(
                                        "Failed to initialize zstd for \
                                         {name}: {e}"
                                    ))
                                })?
                            );
    ...
    let make_archive_fn = lua.create_function(
        move |lua,
            ...
            match source {
                ...
                mlua::Value::Table(t) => {
                    for val in t.sequence_values::<String>() {
                        ...
                        if p.exists() {
                            ...
                            source_paths
                                .push((PathBuf::from(&s_str), s_str.clone()));
                        } else {
                            return Err(mlua::Error::RuntimeError(format!(
                                "MAKE_ARCHIVE: source path does not exist: \
                                 {s_str}"
                ...
                _ => {
                    return Err(mlua::Error::RuntimeError(
                        "MAKE_ARCHIVE: source must be string or table"
                            .to_string()
                    ));
                }
            }
            ...
            match algo.as_str() {
                "gz" => {
                    ...
                    for (path, _) in source_paths {
                        ...
                        std::io::copy(&mut f, &mut encoder).map_err(|e| {
                            mlua::Error::RuntimeError(e.to_string())
                        })?;
                    }
                ...
                "zip" => {
                    ...
                    for (path, rel_name) in source_paths {
                        if path.is_dir() {
                            ...
                            {
                                let rel =
                                    entry.path().strip_prefix(parent).expect(
                                        "entry path should be within source \
                                         path"
                                    );
                                if entry.file_type().is_dir() {
                                    ...
                                    std::io::copy(&mut f, &mut zip).map_err(
                                        |e| {
                                            mlua::Error::RuntimeError(
                                                e.to_string()
                                            )
                ...
                "tar" | "tar.gz" | "tar.xz" | "tar.zst" | "zst" => {
                    ...
                    for (path, rel_name) in source_paths {
                        if path.is_dir() {
                            tar.append_dir_all(rel_name, path).map_err(
                                |e| mlua::Error::RuntimeError(e.to_string())
                            )?;
                        } else {
                            tar.append_path_with_name(path, rel_name).map_err(
                                |e| mlua::Error::RuntimeError(e.to_string())
                            )?;
                        }
                    }
