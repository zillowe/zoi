...
/// - `prepare`/`build`/`package`: Placeholder functions that the maintainer
///   overrides to define the build logic.
/// - `IMPORT`/`INCLUDE`: Helpers for modular package definitions.
///
/// These functions bridge the declarative metadata and the imperative build
/// logic. Adds the `IMPORT` function to the Lua environment for importing data
/// from other files.
///
/// # Errors
///
/// Returns an error if the `IMPORT` function cannot be set in the global
/// environment.
pub fn add_import_util(
    lua: &Lua,
    current_path: &Path,
) -> Result<(), mlua::Error,> {
    ...
    let import_fn = lua.create_function(move |lua, file_name: String| {
        ...
        let parent = current_path_buf.parent().ok_or_else(|| {
            mlua::Error::RuntimeError(
                "Could not determine parent directory of package file"
                    .to_string(),
            )
        },)?;
...
/// environment.
pub fn add_include_util(
    lua: &Lua,
    current_path: &Path,
) -> Result<(), mlua::Error,> {
    let current_path_buf = current_path.to_path_buf();
    let include_fn = lua.create_function(
        move |lua, file_name: String| -> Result<(), mlua::Error,> {
            if let Ok(refs,) =
                lua.globals().get::<Table>("__ZoiReferencedFiles",)
            {
                let _ = refs.push(file_name.clone(),);
            }
            let parent = current_path_buf.parent().ok_or_else(|| {
...
/// # Errors
///
/// Returns an error if any of the lifecycle functions cannot be set in the
/// global environment.
pub fn add_package_lifecycle_functions(lua: &Lua,) -> Result<(), mlua::Error,> {
    ...
    let dependencies_fn =
        lua.create_function(move |lua, deps_def: Table| {
            if let Ok(deps_table,) =
                lua.globals().get::<Table>("__ZoiPackageDeps",)
            {
    ...
    let updates_fn = lua.create_function(move |lua, updates_list: Table| {
        if let Ok(updates_table,) =
            lua.globals().get::<Table>("__ZoiPackageUpdates",)
        {
    ...
    let hooks_fn = lua.create_function(move |lua, hooks_def: Table| {
        if let Ok(hooks_table,) =
            lua.globals().get::<Table>("__ZoiPackageHooks",)
        {
    ...
    let service_fn = lua.create_function(move |lua, service_def: Table| {
        if let Ok(service_table,) =
            lua.globals().get::<Table>("__ZoiPackageService",)
        {
