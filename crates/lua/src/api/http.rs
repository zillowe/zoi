...
/// - `UTILS.FETCH.<FORGE>.LATEST`: Query Git forges (GitHub, GitLab, etc.) for
///   the latest tags, releases, or commit SHAs.
///
...
pub fn add_fetch_util(lua: &Lua,) -> Result<(), mlua::Error,> {
    ...
    let fetch_fn = lua.create_function(
        |_, url: String| -> Result<String, mlua::Error,> {
            ...
            let get_latest_fn =
                lua.create_function(move |lua, args: Table| {
                    let git_args: GitArgs =
                        lua.from_value(Value::Table(args,),).map_err(|e| {
                    ...
                    let base_url =
                        match provider {
                            "GITHUB" => git_args.domain.unwrap_or_else(|| {
                                "https://api.github.com".to_string()
                            },),
                            "GITLAB" => git_args.domain.unwrap_or_else(|| {
                                "https://gitlab.com".to_string()
                            },),
                            "GITEA" => git_args.domain.unwrap_or_else(|| {
                                "https://gitea.com".to_string()
                            },),
                            "FORGEJO" => git_args.domain.unwrap_or_else(|| {
                    ...
                    let url = match (provider, what,) {
                        ("GITHUB", "tag",) => {
                            format!("{base_url}/repos/{}/tags", git_args.repo)
                        }
                        ("GITHUB", "release",) => {
                            format!(
                                "{base_url}/repos/{}/releases/latest",
                                git_args.repo
                            )
                        }
                        ...
                        ("GITLAB", "commit",) => format!(
                            "{base_url}/api/v4/projects/{}/repository/commits?\
                             ref_name={}",
                            urlencoding::encode(&git_args.repo),
                            git_args.branch.as_deref().unwrap_or("HEAD")
                        ),
                    ...
                    result.map(std::string::ToString::to_string,).ok_or_else(
                        || {
                            mlua::Error::RuntimeError(
                                "Could not extract value from API response"
                                    .to_string(),
                            )
                        },
                    )
