...
/// # Errors
///
/// Returns an error if the local keyring directory cannot be created or accessed.
...
pub fn add_key_from_bytes(key_bytes: &[u8], name: &str, quiet: bool) -> Result<()> {
    ...
    if dest_path.exists() {
        ...
        if !quiet {
            println!(
                "{} A different key with the name '{name}' already exists. Overwriting.",
                "Warning:".yellow(),
            );
        }
    }
...
/// # Errors
///
/// Returns an error if the key cannot be fetched from the keyserver or is invalid.
...
/// # Errors
///
/// Returns an error if the key with the given name is not found or cannot be removed.
...
/// # Errors
///
/// Returns an error if no key with the given fingerprint is found or cannot be removed.
...
/// # Errors
///
/// Returns an error if the local keyring cannot be read or contains invalid keys.
pub fn get_all_local_keys_info() -> Result<Vec<KeyInfo>> {
    ...
    for entry in fs::read_dir(pgp_dir)? {
            ...
            && let Ok(cert) = Cert::from_bytes(&bytes)
        {
            let name = path
                .file_stem()
                .ok_or_else(|| anyhow!("Path should have a file stem: {}", path.display()))?
                .to_string_lossy()
                .to_string();
...
pub fn cli_verify_signature(file_path: &str, sig_path: &str, key_name: &str) -> Result<()> {
    println!(
        "Verifying {file_path} with signature {sig_path} using key '{key_name}'"
    );
...
/// # Errors
///
/// Returns an error if `gpg` is not installed, the file cannot be read, or signing fails.
pub fn sign_detached(data_path: &Path, signature_path: &Path, key_id: &str) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        if stderr.contains("No secret key") {
            let _ = writeln!(
                error_message,
                "The secret key for '{key_id}' was not found in your GPG keychain."
            );
...
/// # Errors
///
/// Returns an error if any of the trusted keys are not found in the local keyring.
