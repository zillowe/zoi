...
///
/// A Sysroot allows Zoi to operate on an external directory as if it were
/// the primary system root (`/`). This is a powerful feature for:
/// - Bootstrapping: Creating a bootable OS image or container from the
///   outside.
/// - Cross-Installation: Installing packages for an embedded system or
///   another partition.
/// - Isolation: Testing installations without affecting the current host
///   system.
///
...
/// If the path is absolute, it is made relative to the current root before
/// joining.
