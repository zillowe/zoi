...
#[derive(
    Debug, Serialize, Deserialize, PartialEq, Eq, Clone, Copy, Default,
)]
...
pub struct PooledZpaManifest {
    ...
    pub mappings: BTreeMap<String, SubPackageMapping> /* sub_package ->
                                                       * mapping */
}
...
/// `.pkg.lua` file. It defines what a package is, where it comes from, and how
/// it can be installed, but does not represent an actual installation on disk
/// (see `InstallManifest` for that).
#[derive(Debug, Serialize, Deserialize, Clone, Default)]
#[allow(dead_code)]
pub struct Package {
    ...
    #[serde(default)]
    pub epoch: u32,
    /// Incremental revision for the same upstream version (e.g. for packaging
    /// fixes).
...
pub struct DependencyOptionGroup {
    ...
    pub desc: String,
    /// Whether all dependencies in the group should be installed if the group
    /// is selected.
    #[serde(default)]
    pub all: bool,
...
/// A simplified version of an install manifest that can be easily shared or
/// compared.
#[derive(Debug, Serialize, Deserialize, Clone, PartialEq, Eq)]
...
/// The root structure of a `packages.json` registry index file (Specification
/// v2).
///
/// This file is generated by `zoi registry generate-metadata` and acts as a
/// highly optimized, centralized index of every `.pkg.lua` file in a registry.
...
pub struct RegistryIndexV2 {
    ...
    pub version: String,
    /// A map of package identifiers (`@repo/name`) to their pre-computed
    /// metadata.
...
/// - Local State Hashes: Cryptographic hashes of the actual store and database
///   directories.
/// - Registry Revisions: The exact Git commit SHAs of the registries used to
///   resolve packages.
...
pub struct ZoiLockV2 {
    ...
    pub version: String,
    /// The SHA-512 hash of the `.zoi/pkgs/store` directory, capturing the
    /// exact file state.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub packages_hash: Option<String>,
    /// The SHA-512 hash of the `.zoi/pkgs/db` directory, capturing the
    /// registry metadata state.
    #[serde(default, skip_serializing_if = "Option::is_none")]
