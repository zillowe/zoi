...
///
/// This represents a requirement for another package, which may be managed by
/// Zoi or an external package manager (e.g. `apt`, `brew`, `npm`).
#[derive(Debug,)]
pub struct Dependency<'a,> {
    /// The name of the package manager responsible for this dependency (e.g.
    /// "zoi", "apt").
    pub manager: &'a str,
...
/// Regex for parsing a dependency string into package+version and an optional
/// description.
...
/// Regex for parsing a package+version string into package name and version
/// requirement.
...
/// - `is_known_manager`: A closure that determines if a prefix should be
///   treated as a manager name.
///
/// # Errors
///
/// Returns an error if the dependency string is invalid, the package name is
/// missing, or if the version requirement is malformed for the "zoi" manager.
pub fn parse_dependency_string(
    ...
    let package_and_version = caps
        .name("pkg_and_ver",)
        .ok_or_else(|| {
            anyhow!("Regex matched but pkg_and_ver group not found in '{rest}'")
        },)?
        .as_str()
        .trim();
    ...
    let ver_caps = VER_RE.captures(package_and_version,).ok_or_else(|| {
        anyhow!(
            "Failed to parse package and version from: {package_and_version}"
        )
    },)?;
    ...
    let package = ver_caps
        ...
        .trim();
    let mut version_str =
        ver_caps.name("ver",).map(|m| m.as_str().to_string(),);
