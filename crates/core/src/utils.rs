...
/// Returns a standard Zoi platform identifier (e.g. "linux-amd64",
/// "windows-arm64").
///
...
/// Detects the general family of a Linux distribution (e.g. "debian", "arch",
/// "fedora").
///
...
///   This is the most reliable way to identify derivatives (e.g. Ubuntu is
///   `debian`).
/// - Direct ID Match: If `ID_LIKE` is missing, we fall back to the primary
///   `ID`.
...
/// # Errors
///
/// Returns an error if the owner or group lookup fails, or if the chown
/// operation fails.
#[cfg(unix)]
...
pub fn confirm_untrusted_source(
    ...
    let warning_message = match source_type {
        ...
        crate::types::SourceType::LocalFile => {
            "You are installing from a local file.".to_string()
        }
        crate::types::SourceType::Url => {
            "You are installing from a remote URL. This script will be \
             executed with your user's permissions, which could lead to remote \
