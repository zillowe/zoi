...
/// Recursively collects all dependencies for a package, including sub-package
/// specific ones.
///
/// This function handles the complex logic of:
/// - Required Dependencies: Automatically added to the list.
/// - Optional Dependencies: Prompts the user (unless `--all-optional` or
///   `--yes` is used).
/// - Selectable Options: Prompts the user to choose between multiple providers.
/// - Recursive Sub-packages: Follows `sub_packages` maps to pull in
///   component-specific requirements.
///
/// # Errors
///
/// Returns an error if any of the underlying dependency collection or prompting
/// operations fail.
...
/// # Errors
///
/// Returns an error if user interaction via the terminal fails or if a
...
pub fn prompt_for_optionals(
    ...
    if yes {
        println!(
            "{} Skipping optional {}dependencies (--yes provided without \
             --all-optional).",
            "::".bold().yellow(),
            type_str
        );
