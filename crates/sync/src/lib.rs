...
use walkdir::WalkDir;
use zoi_core::{config, offline, pgp, types, utils as core_utils};
use zoi_db as db;
...
/// - Fetches remote metadata (sizes and file lists) if configured in
///   `repo.yaml`.
...
fn refresh_registry_db(
        ...
        .filter_map(|path| {
                ...
                lua_parser::parse_lua_package(&path_str, None, None, true,)
            {
                ...
                if let Some(c,) = &client
                    ...
                    && let Some(pkg_link,) =
                        rc.pkg.iter().find(|p| p.link_type == "main",)
    ...
    let parsed_advisories: Vec<(types::Advisory, String,),> = sec_files
        ...
        .filter_map(|path| {
            if let Ok(content,) = fs::read_to_string(path,)
                && let Ok(advisory,) =
                    serde_yaml::from_str::<types::Advisory,>(&content,)
                && let Ok(rel_path,) = path.strip_prefix(registry_path,)
                && let Some(parent,) = rel_path.parent()
            {
                let repo_path =
    ...
    for (pkg, _path, file_list, size_info, hash_info,) in parsed_results {
        let pkg_id =
            db::update_package(&tx, &pkg, registry_handle, None, None, None,)?;

        if let Some((down_size, install_size,),) = size_info {
            let _ =
                db::set_package_sizes(&tx, pkg_id, down_size, install_size,);
        }
        ...
        if let Some(subs,) = &pkg.sub_packages {
            for sub in subs {
                if let Err(e,) = db::update_package(
                    &tx,
                    &pkg,
                    registry_handle,
                    None,
                ...
                } else if let Ok(sub_id,) = db::get_package_id(
                    &tx,
                    &pkg.name,
                    Some(sub,),
                    &pkg.repo,
...
///
/// This ensures that the registry state hasn't been tampered with on the
/// server. If verification fails, the sync is aborted for security reasons.
fn verify_registry_signature(
    ...
    let (sig, data,) =
        repo.extract_signature(&commit.id(), None,).map_err(|_| {
            anyhow!("Registry commit is not signed. Sync aborted for security.")
        },)?;
    ...
    for cert in trusted_certs {
        if pgp::verify_detached_signature_raw(data_bytes, sig_bytes, &cert,)
            .is_ok()
        {
            verified = true;
            break;
    ...
    if verified {
        ...
        Err(anyhow!(
            "Registry commit was signed but not by any authorized authority. \
             Sync aborted."
        ),)
    }
}
...
fn sync_git_repos(verbose: bool, scope: types::Scope,) -> Result<(),> {
    if offline::is_offline() {
        println!(
            "\n{}",
            "Zoi is offline. Skipping sync of external git repositories."
                .yellow()
        );
        return Ok((),);
    }
...
fn run_non_verbose_at_path(
    ...
    let fetch_style = ProgressStyle::default_bar()
        .template(
            "{spinner:.green} [{elapsed_precise}] {msg:30.cyan} \
    ...
    if db_path.exists() {
        ...
        let short_branch_name = remote_default_ref
            ...
            .ok_or_else(|| {
                anyhow!(
                    "Could not determine default branch name from remote HEAD"
                )
            },)?;
...
/// Attempts to sync a repository, handling local paths and falling back to
/// system git if needed.
fn try_sync_at_path(
    db_url: &str,
    db_path: &Path,
    verbose: bool,
    ...
    if local_path.is_dir() {
        ...
        // For local registries, we ensure db_path is a symlink to the
        // local_path
        if db_path.exists() {
    ...
    if offline::is_offline() {
        ...
        return Err(anyhow!(
            "Cannot sync registry '{db_url}': Zoi is offline and registry is \
             not cloned."
        ),);
    }
    if db_path.exists()
    ...
    if verbose {
        ...
        match run_non_verbose_at_path(db_url, db_path, m, pb,) {
            ...
            Err(libgit_error,) => {
                let msg = format!(
                    "Git progress sync failed for {}: {}. Retrying with \
                     system git...",
                    db_url.yellow(),
                    libgit_error
                );
...
/// Clones a repository to a temporary directory to read its repo.yaml and get
/// its handle.
...
fn fetch_repo_yaml_content(url: &str,) -> Result<String,> {
    let (provider, repo_path,) =
        parse_full_repo_url(url,).ok_or_else(|| {
...
fn fetch_handle_for_url(url: &str, verbose: bool,) -> Result<String,> {
    ...
    match fetch_repo_yaml_content(url,) {
        ...
        Err(e,) => {
            if verbose {
                println!(
                    "Direct fetch failed: {}. Falling back to cloning \
                     repository...",
                    e.to_string().yellow()
                );
            }
...
fn sync_registry(
    ...
    if sync_success {
        ...
        {
            let platform = core_utils::get_platform().unwrap_or_default();
            let db_url = install_util::resolve_url_placeholders(
                db_url_template,
                "",
                "",
                "",
    ...
    let results: Vec<((String, String,), String,),> = registries
        ...
        .map(|(handle, url, revision,)| {
            ...
            if !revision.is_empty() {
                if verbose {
                    println!(
                        "  Checking out revision '{revision}' for registry \
                         '{handle}'..."
                    );
                }
                let status = Command::new("git",)
                ...
                if !status.success() {
                    return Err(anyhow!(
                        "Failed to checkout revision '{revision}' for \
                         registry '{handle}'"
                    ),);
                }
            }
...
pub fn run(
    ...
    if force {
        println!(
            "{} Force sync: removing existing databases and re-syncing from \
             scratch...",
            "::".bold().yellow()
        );
    }
    ...
    if !registries_to_sync.is_empty() {
        ...
        let results: Vec<Result<(types::Registry, bool, bool,),>,> =
            registries_to_sync
                .into_par_iter()
                .map(|(reg, is_default,)| {
                    let (synced_reg, changed,) = sync_registry(
                        reg,
                        &db_root,
                        verbose,
                        fallback,
    ...
    if !no_pm {
        ...
        config.native_package_manager =
            core_utils::get_native_package_manager();
        config.package_managers =
            Some(core_utils::get_all_available_package_managers(),);
        needs_config_update = true;
        if verbose {
