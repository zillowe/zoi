...
fn test_zoinew_upgrade_logic() {
    ...
    fs::write(&old_config_path, "user-custom-setting: true",)
        .expect("unwrap failed",);

    fs::write(&new_config_path, "user-custom-setting: false",)
        .expect("unwrap failed",);
    ...
    if old_config_path.exists() {
        if new_config_path.exists() {
            let extension = new_config_path
                ...
                .unwrap_or_default();
            let zoinew_path =
                new_config_path.with_extension(format!("{extension}.zoinew"),);
            fs::rename(&new_config_path, &zoinew_path,)
                .expect("unwrap failed",);
        }
...
fn test_zoisave_uninstall_logic() {
    ...
    assert_eq!(
        fs::read_to_string(backup_dest).expect("unwrap failed"),
        "final-user-data"
    );
}
