...
fn test_hash_verification() {
    ...
    let calculated = helper::get_hash(
        file_path.to_str().expect("unwrap failed",),
        helper::HashType::Sha512,
    )
    .expect("unwrap failed",);
    let second_run = helper::get_hash(
        file_path.to_str().expect("unwrap failed",),
        helper::HashType::Sha512,
    )
    .expect("unwrap failed",);
    ...
    let calculated_sha256 = helper::get_hash(
        file_path.to_str().expect("unwrap failed",),
        helper::HashType::Sha256,
    )
    .expect("unwrap failed",);
...
fn test_directory_hashing() {
    ...
    fs::write(sub.join("b.txt",), "content-b-changed",)
        .expect("unwrap failed",);
