...
fn test_get_conflicts_from_list_detects_existing_files() {
    ...
    assert_eq!(conflicts.first().expect("Value should exist in test").as_str(), conflicting_file.to_string_lossy().to_string());
}
...
fn test_get_conflicts_from_list_ignores_different_scopes() {
    ...
    fs::create_dir_all(sys_file.parent().expect("unwrap failed")).expect("unwrap failed");
