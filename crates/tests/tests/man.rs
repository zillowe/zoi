...
fn test_man_resolution_by_provides() {
    ...
    let pkg_id =
        db::update_package(&conn, &pkg, handle, Some(types::Scope::User), None, None).expect("unwrap failed");
    db::index_package_files(&conn, pkg_id, &[format!("data/pkgstore/bin/{bin_name}")]).expect("unwrap failed");
    ...
    fs::write(version_dir.join("man.md"), "# Manual Content").expect("unwrap failed");

    let latest_path = pkg_path.join("latest");
    zoi::utils::symlink_file(&version_dir, &latest_path).expect("unwrap failed");

    let (resolved_pkg, resolved_handle) = man::resolve_package_for_man(bin_name).expect("unwrap failed");
...
fn test_gather_local_manual_pages() {
    ...
    fs::write(share_man.join("tool.1"), ".TH TOOL 1\n.SH NAME\ntool").expect("unwrap failed");
    fs::write(share_man.join("extra.md"), "# Extra").expect("unwrap failed");

    let latest_path = pkg_path.join("latest");
    zoi::utils::symlink_file(&version_dir, &latest_path).expect("unwrap failed");

    let pages = man::gather_manual_pages(&pkg, Some(handle), false, true).expect("unwrap failed");
    assert_eq!(pages.len(), 2);
    assert!(pages.contains_key("tool.1"));
    assert!(pages.contains_key("extra.md"));
    assert!(pages.get("tool.1").expect("unwrap failed").contains("# TOOL"));
    assert!(pages.get("extra.md").expect("unwrap failed").contains("# Extra"));
}
