...
fn test_shim_resolution_logic() {
    ...
    let providers = db::find_provides("local", bin_name).expect("unwrap failed");
    println!(
        "Providers for '{}': {:?}",
        bin_name,
    ...
    let resolved = shim::resolve_to_installed_bin(bin_name, Some(&pm), None).expect("unwrap failed");
    ...
    let resolved_v2 = shim::resolve_to_installed_bin(bin_name, Some(&pm), None).expect("unwrap failed");
