...
fn test_shim_resolution_logic() {
    ...
    let pkg_id =
        zoi::pkg::utils::generate_package_id("local", "core", pkg_name,);
    let pkg_dir_name =
        zoi::pkg::utils::get_package_dir_name(&pkg_id, pkg_name,);
    let store_base =
    ...
    let providers =
        db::find_provides("local", bin_name,).expect("unwrap failed",);
    println!(
        "Providers for '{}': {:?}",
        bin_name,
    ...
    let resolved = shim::resolve_to_installed_bin(bin_name, Some(&pm,), None,)
    ...
    let resolved_v2 =
        shim::resolve_to_installed_bin(bin_name, Some(&pm,), None,)
            .expect("unwrap failed",);
    println!("Resolved v2 override: {}", resolved_v2.display());
    let canonical_v2 =
        resolved_v2.canonicalize().unwrap_or(resolved_v2.clone(),);
