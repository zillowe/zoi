...
fn test_find_provides_logic() {
    ...
    db::index_package_files(&conn, pkg_id, &["data/pkgstore/bin/git".to_string()]).expect("unwrap failed");

    let results = db::find_provides(handle, "git").expect("unwrap failed");
    assert!(!results.is_empty());
    assert_eq!(results.first().expect("Value should exist in test").0.name, "git");
}
