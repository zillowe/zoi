...
fn test_helper_get_hash_sha256() {
    ...
    let hash = helper::get_hash(file_path, helper::HashType::Sha256)
        .expect("get_hash failed");
    assert_eq!(
        hash,
...
fn test_helper_get_hash_sha512() {
    ...
    let hash = helper::get_hash(file_path, helper::HashType::Sha512)
        .expect("get_hash failed");
    assert_eq!(
        hash,
...
fn test_helper_validate_packages_json() {
    let res =
        helper::validate::run(&PathBuf::from("tests/assets/packages.json"));
    assert!(res.is_ok(), "Validation failed: {res:?}");
}
...
fn test_helper_validate_advisories_json() {
    let res =
        helper::validate::run(&PathBuf::from("tests/assets/advisories.json"));
    assert!(res.is_ok(), "Validation failed: {res:?}");
}

#[test]
fn test_helper_validate_sec_yaml() {
    let res = helper::validate::run(&PathBuf::from(
