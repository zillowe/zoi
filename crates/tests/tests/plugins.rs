...
fn test_plugin_fs_symlink_api() {
    ...
    fs::write(&target, "hello").expect("unwrap failed");

    let pm = plugin::PluginManager::new().expect("unwrap failed");
    let script = format!(
        r#"return zoi.fs.symlink("{}", "{}", false)"#,
        target.to_str().expect("unwrap failed").replace('\\', "/"),
        link.to_str().expect("unwrap failed").replace('\\', "/")
    );

    let success: bool = pm.lua.load(&script).eval().expect("unwrap failed");
    assert!(success, "Symlink API should return true");
    assert!(link.exists(), "Link should exist");
    assert_eq!(fs::read_to_string(link).expect("unwrap failed"), "hello");
}
...
fn test_plugin_archive_extract_api() {
    ...
    fs::create_dir_all(source_dir.join("package")).expect("unwrap failed");
    fs::write(source_dir.join("package/hello.txt"), "world").expect("unwrap failed");

    {
        let file = fs::File::create(&tar_path).expect("unwrap failed");
        let enc = flate2::write::GzEncoder::new(file, flate2::Compression::default());
        let mut tar = tar::Builder::new(enc);
        tar.append_dir_all("package", source_dir.join("package"))
            .expect("unwrap failed");
        tar.finish().expect("unwrap failed");
    }
    ...
    let script = format!(
        ...
        tar_path.to_str().expect("unwrap failed").replace('\\', "/"),
        dest.to_str().expect("unwrap failed").replace('\\', "/")
    );
    ...
    assert_eq!(fs::read_to_string(dest.join("hello.txt")).expect("unwrap failed"), "world");
}
...
fn test_plugin_archive_extract_rejects_path_traversal_with_strip() {
    ...
    let zip_path = root.join("evil.zip");
    {
        ...
        let file = fs::File::create(&zip_path).expect("unwrap failed");
        ...
        .expect("unwrap failed");
        zip.write_all(b"escape").expect("unwrap failed");
        zip.finish().expect("unwrap failed");
    }
    ...
    let script = format!(
        r#"return zoi.archive.extract("{}", "{}", 1)"#,
        zip_path.to_str().expect("unwrap failed").replace('\\', "/"),
        dest.to_str().expect("unwrap failed").replace('\\', "/")
    );
...
fn test_plugin_fs_copy_api() {
    ...
    fs::create_dir_all(&src).expect("unwrap failed");
    fs::write(src.join("file.txt"), "copy-content").expect("unwrap failed");

    let pm = plugin::PluginManager::new().expect("unwrap failed");
    let script = format!(
        r#"return zoi.fs.copy("{}", "{}")"#,
        src.to_str().expect("unwrap failed").replace('\\', "/"),
        dest.to_str().expect("unwrap failed").replace('\\', "/")
    );
    ...
    assert_eq!(
        fs::read_to_string(dest.join("file.txt")).expect("unwrap failed"),
        "copy-content"
    );
}
...
fn test_plugin_sh_api() {
    ...
    let script = format!(
        ...
        out_file.to_str().expect("unwrap failed").replace('\\', "/")
    );

    let exit_code: i32 = pm.lua.load(&script).eval().expect("unwrap failed");
    assert_eq!(exit_code, 0, "zoi.sh should return 0 exit code");
    assert!(out_file.exists());
    assert_eq!(fs::read_to_string(out_file).expect("unwrap failed").trim(), "sh-works");
}
...
"#,
    ...
    for file in &["a-first.lua", "z-last.lua"] {
        let content = fs::read_to_string(plugin_dir.join(file)).expect("unwrap failed");
    ...
    fs::write(
        ...
        serde_json::to_string(&trusted).expect("unwrap failed"),
    )
    .expect("unwrap failed");

    let pm = plugin::PluginManager::new().expect("unwrap failed");
    pm.load_all(true).expect("unwrap failed");

    let handled = pm.trigger_project_install_hook().expect("unwrap failed");
    assert!(
        handled,
...
fn test_plugin_on_project_install_precedence() {
    ...
    let count: i32 = pm.lua.load("return get_count()").eval().expect("unwrap failed");
    assert_eq!(
        count, 1,
