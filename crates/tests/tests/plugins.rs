...
fn test_plugin_on_project_install_hook() {
    ...
    let pm =
        plugin::PluginManager::new().expect("Failed to create PluginManager");
...
fn test_plugin_archive_extract_api() {
    ...
    fs::write(source_dir.join("package/hello.txt"), "world")
    ...
    assert_eq!(
        fs::read_to_string(dest.join("hello.txt")).expect("unwrap failed"),
        "world"
    );
}
...
fn test_plugin_sh_api() {
    ...
    assert_eq!(
        fs::read_to_string(out_file).expect("unwrap failed").trim(),
        "sh-works"
    );
}
...
fn test_plugin_set_data_rejects_non_finite_number() {
    ...
    let result: Result<(), mlua::Error> =
        pm.lua.load(r#"zoi.set_data("bad", 0/0)"#).exec();
    assert!(
        result.is_err(),
...
"#
    ...
    for file in &["a-first.lua", "z-last.lua"] {
        let content =
            fs::read_to_string(plugin_dir.join(file)).expect("unwrap failed");
