...
fn test_parse_mini_registry_index() {
    ...
    let index: MiniRegistryIndex = serde_json::from_str(&content).expect("unwrap failed");

    assert!(index.packages.contains_key("hello"));
    let hello = &index.packages.get("hello").expect("Value should exist in test");
    assert_eq!(hello.repo, "zillowe");
    assert_eq!(hello.repo_type, "official");
    assert_eq!(hello.version, "4.0.0");
    assert_eq!(hello.vuln.as_ref().expect("unwrap failed").len(), 1);
    assert_eq!(hello.vuln.as_ref().expect("unwrap failed").first().expect("Value should exist in test").id, "ZSA-2026-D0042");

    assert!(index.packages.contains_key("collision"));
    assert_eq!(index.packages.get("collision").expect("Value should exist in test").repo, "extra");
}
...
fn test_parse_mini_registry_config() {
    ...
    let content = fs::read_to_string(path).expect("unwrap failed");
    let config: zoi::pkg::types::RepoConfig = serde_yaml::from_str(&content).expect("unwrap failed");
...
fn test_mini_vulnerability_check() {
    ...
    assert!(check_vulnerabilities("test", &pkg_info, "2.0.0").expect("unwrap failed"));
}
