...
fn split_explicit_file_source(
    ...
    let (main_part, version_spec) =
        if let Some((base, version)) = source_str.rsplit_once('@') {
...
fn find_package_in_db(
    ...
    fn process_found_package(
        ...
        let repo_type = if let Some(ref cfg) = repo_config {
            cfg.repos.iter().find(|r| r.name == major_repo).map_or_else(
...
fn download_from_url(url: &str) -> Result<ResolvedSource> {
    let (base_url, expected_hash) = if let Some((base, hash_part)) =
        url.split_once('#')
    {
...
fn resolve_source_recursive(
    ...
    let resolved_source = if source.starts_with("#git@") {
        ...
        let repo_name =
            parts.first().ok_or_else(|| anyhow!("Invalid git source"))?;
