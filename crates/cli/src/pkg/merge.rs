...
/// Performs a 3-way merge on configuration files defined in the `backup`
/// manifest field.
///
/// It compares:
...
///
/// Merging logic:
/// - If Yours == Base: User didn't change it. Use Theirs (do nothing, already
///   in place).
/// - If Theirs == Base: Upstream didn't change it. Use Yours (copy Yours over
///   Theirs).
...
pub fn handle_backup_files(
    ...
    for backup_file_rel in backup_files {
        let old_expanded = crate::pkg::utils::expand_placeholders(
            backup_file_rel,
            old_version_dir,
            scope
        )?;
        let new_expanded = crate::pkg::utils::expand_placeholders(
            backup_file_rel,
            new_version_dir,
            scope
        )?;
        ...
        if old_path.exists() {
                ...
                )
            {
                ...
                if theirs == base {
                    ...
                    if let Err(e) = fs::copy(&old_path, &new_path) {
                        eprintln!(
                            "Warning: failed to restore user config: {e}"
                        );
                    }
                    continue;
                }
                ...
                match merge(&base, &yours, &theirs) {
                    Ok(merged) => {
                        ...
                        if let Err(e) = fs::write(&new_path, merged) {
                            eprintln!(
                                "Warning: failed to write merged config: {e}"
                            );
                        }
                    }
                    Err(conflicted) => {
                        ...
                        if let Err(e) = fs::write(&new_path, conflicted) {
                            eprintln!(
                                "Warning: failed to write conflicted config: \
                                 {e}"
                            );
                        }
                    }
            ...
            if new_path.exists() {
                ...
                println!(
                    "Configuration file '{}' exists in new version. Saving as \
                     .zoinew",
                    new_path.display()
                );
...
mod tests {
    ...
    fn test_merge_case_a_unmodified() {
        ...
        fs::write(old_dir.join("config.txt.zoiorig"), base)
            .expect("Failed to write base");
        fs::write(old_dir.join("config.txt"), base)
        ...
        assert_eq!(
            fs::read_to_string(new_dir.join("config.txt"))
    ...
    fn test_merge_case_b_upstream_unchanged() {
        ...
        fs::write(old_dir.join("config.txt"), "line1\nline2\nuser_mod\n")
            .expect("Failed to write yours");
        fs::write(new_dir.join("config.txt"), base)
            .expect("Failed to write theirs");
    ...
    fn test_merge_case_c_clean_merge() {
        ...
        let result = fs::read_to_string(new_dir.join("config.txt"))
            .expect("Failed to read result");
