...
/// # Errors
///
/// Returns an error if the package is not installed or if there is an error
/// reading the package metadata or dependents.
pub fn run(package_name: &str,) -> Result<(),> {
    ...
    let manifest = crate::cmd::installed_select::choose_installed_manifest(
        package_name,
        &candidates,
        false,
    )?;
    ...
    if reasons.is_empty() {
        if matches!(manifest.reason, types::InstallReason::Dependency { .. }) {
            println!(
                "Package '{}' is installed as a dependency, but no packages \
                 list it as a requirement. It may be an orphan.",
                local::installed_manifest_source(&manifest).bold()
            );
        } else {
            println!(
                "Package '{}' is installed, but its installation reason is \
                 unclear.",
                local::installed_manifest_source(&manifest).bold()
            );
        }
