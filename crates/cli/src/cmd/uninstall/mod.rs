...
pub fn run(
    ...
    if save
        && let Err(e) = zoi_project::config::remove_packages_from_config(
            &successfully_uninstalled
        )
    {
        eprintln!(
            "{}: Failed to remove packages from zoi.lua: {}",
            "Warning".yellow().bold(),
            e
        );
    }
...
/// # Errors
///
/// Returns an error if the package name is invalid or if the package is not
/// installed.
...
/// Recursively finds dependency-reason packages that would become orphaned if
/// the target packages were removed.
///
/// # Errors
///
