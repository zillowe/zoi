...
fn gather_manual_pages_from_upstream(
    ...
    if legacy_man.exists() {
        pages.extend(find_local_man_pages(
            &extract_path.join("data/pkgstore",),
        )?,);
    }
...
fn find_local_man_pages(
    ...
    let search_dirs = [
        latest_dir.join("share",).join("man",),
        latest_dir.join("man",),
    ];
        ...
        match event {
            CmarkEvent::Start(tag,) => match tag {
                Tag::Heading { level, .. } => {
                    ...
                    current_line
                        .push(Span::raw("#".repeat(level_num,) + " ",),);
                }
                Tag::BlockQuote(_,) => {
                ...
                Tag::Item => {
                    ...
                    if let Some((index, _,),) = list_stack.last_mut() {
                        ...
                        current_line
                            .push(Span::raw("  ".repeat(list_len - 1,),),);
