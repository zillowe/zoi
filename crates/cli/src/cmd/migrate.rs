...
//!
//! This command facilitates migrating packages from other formats (like Scoop)
//! to Zoi.
...
fn run_scoop(args: &ScoopCommand,) -> Result<(),> {
    let content = std::fs::read_to_string(&args.input,).map_err(|e| {
        anyhow!("Failed to read '{}': {}", args.input.display(), e)
    },)?;
    ...
    let license = get_license_field(manifest_obj,)
    ...
    std::fs::write(&output, lua,).map_err(|e| {
        anyhow!("Failed to write '{}': {}", output.display(), e)
    },)?;
...
fn derive_name_from_path(path: &Path,) -> Result<String,> {
    let stem = path.file_stem().and_then(|s| s.to_str(),).ok_or_else(|| {
        anyhow!("Failed to derive package name from '{}'.", path.display())
    },)?;
    Ok(stem.to_string(),)
}
...
fn default_output_path(input: &Path, name: &str,) -> PathBuf {
    match input.parent() {
        Some(parent,) => parent.join(format!("{name}.pkg.lua"),),
        None => PathBuf::from(format!("{name}.pkg.lua"),),
    }
}

/// Normalizes a package name by trimming, lowercasing, and replacing spaces
/// with hyphens.
...
/// Extracts the license field from a JSON map, handling both string and object
/// formats.
fn get_license_field(
    map: &serde_json::Map<String, Value,>,
) -> Option<String,> {
...
fn collect_bins(
    ...
    if let Some(arch_obj,) =
        manifest.get("architecture",).and_then(Value::as_object,)
    {
...
fn parse_bin_value(value: Option<&Value,>,) -> Vec<BinMapping,> {
    ...
    match value {
        ...
        Value::Array(items,) => {
            let all_strings = items.iter().all(serde_json::Value::is_string,);
...
/// Returns the file stem or basename of a path, stripping common executable
/// extensions.
...
fn collect_download_specs(
    map: &serde_json::Map<String, Value,>,
) -> Result<Vec<DownloadSpec,>,> {
    ...
    if let Some(arch,) = map.get("architecture",).and_then(Value::as_object,) {
        ...
        for key in keys {
            let arch_data =
                arch.get(&key,).and_then(Value::as_object,).ok_or_else(
                    || anyhow!("architecture.{key} must be an object."),
                )?;
    ...
    if specs.is_empty() {
        return Err(anyhow!(
            "Scoop manifest is missing download URL fields (`url` or \
...
fn collect_hook_scripts(map: &serde_json::Map<String, Value,>,) -> HookScripts {
    ...
    if let Some(installer_obj,) =
        map.get("installer",).and_then(Value::as_object,)
    {
        hooks
            .post_install
            .extend(value_to_string_list(installer_obj.get("script",),),);
    }
    if let Some(uninstaller_obj,) =
        map.get("uninstaller",).and_then(Value::as_object,)
    {
        hooks
            .pre_remove
    ...
    if let Some(arch,) = map.get("architecture",).and_then(Value::as_object,) {
        for arch_data in arch.values().filter_map(Value::as_object,) {
            ...
            hooks.post_remove.extend(value_to_string_list(
                arch_data.get("post_uninstall",),
            ),);

            if let Some(installer_obj,) =
                arch_data.get("installer",).and_then(Value::as_object,)
            {
                hooks.post_install.extend(value_to_string_list(
                    installer_obj.get("script",),
                ),);
            }
            if let Some(uninstaller_obj,) =
                arch_data.get("uninstaller",).and_then(Value::as_object,)
            {
                hooks.pre_remove.extend(value_to_string_list(
                    uninstaller_obj.get("script",),
                ),);
            }
        }
    }
...
fn collect_migration_notes(
    ...
    if manifest.get("checkver",).is_some() {
        notes.push(
            "`checkver` is preserved in `SCOOP_MANIFEST` for maintainer \
             automation, but has no direct runtime equivalent in Zoi."
                .to_string(),
        );
    }
    if manifest.get("autoupdate",).is_some() {
        notes.push(
            "`autoupdate` is preserved in `SCOOP_MANIFEST`; update automation \
             remains a maintainer workflow."
                .to_string(),
        );
    }
    if manifest.get("shortcuts",).is_some() {
        notes.push(
            "`shortcuts` are preserved in `SCOOP_MANIFEST`; shortcut creation \
             is not auto-generated in this migration."
                .to_string(),
        );
    }
    if manifest.get("env_set",).is_some()
        || manifest.get("env_add_path",).is_some()
    {
        notes.push(
            "Environment modifications (`env_set` / `env_add_path`) are \
             preserved in `SCOOP_MANIFEST` and should be reviewed manually."
                .to_string(),
        );
    }
    if manifest.get("suggest",).is_some() {
        notes.push(
            "`suggest` is preserved in `SCOOP_MANIFEST` for manual review; it \
             is not mapped to hard dependencies."
                .to_string(),
        );
    }
    if manifest.get("psmodule",).is_some() {
        notes.push(
            "`psmodule` is preserved in `SCOOP_MANIFEST`; validate module \
             installation behavior manually."
                .to_string(),
        );
    }
    if manifest.get("innosetup",).is_some() || manifest.get("msi",).is_some() {
        notes.push(
            "Installer metadata (`innosetup` / `msi`) is preserved in \
             `SCOOP_MANIFEST`; verify installer flow manually."
                .to_string(),
        );
    }
    if let Some(installer,) =
        manifest.get("installer",).and_then(Value::as_object,)
        && (installer.get("file",).is_some()
            || installer.get("args",).is_some())
    {
        notes.push(
            "`installer.file` / `installer.args` are preserved in \
             `SCOOP_MANIFEST`; they are not auto-translated into package() \
             steps."
                .to_string(),
        );
    }
    if let Some(uninstaller,) =
        manifest.get("uninstaller",).and_then(Value::as_object,)
        && (uninstaller.get("file",).is_some()
            || uninstaller.get("args",).is_some())
    {
        notes.push(
            "`uninstaller.file` / `uninstaller.args` are preserved in \
             `SCOOP_MANIFEST`; they are not auto-translated into uninstall \
             hooks."
                .to_string(),
        );
    }
    if bins.iter().any(|b| !b.args.is_empty(),) {
        notes.push(
            "Bin shim extra args are preserved in BIN_MAP but not \
             automatically applied in generated `bins` metadata."
                .to_string(),
        );
    }
    if downloads.len() > 1 {
        notes.push(
            "Multiple architecture/source entries detected. Review generated \
             arch selection and extraction paths."
                .to_string(),
        );
    }
    if manifest.get("cookie",).is_some() {
        notes.push(
            "`cookie` is preserved in `SCOOP_MANIFEST`; authenticated \
             download behavior may require manual adaptation."
                .to_string(),
        );
    }
    if notes.is_empty() {
        notes.push(
            "No major migration warnings detected. Review generated paths and \
             hooks before publishing."
                .to_string(),
        );
    }
...
fn render_pkg_lua(g: &Generated,) -> String {
    ...
    for spec in &g.downloads {
        let _ = writeln!(out, "  [{}] = {{", lua_quote(&spec.arch_key));
        out.push_str("    urls = {",);
    ...
    for bin in &g.bins {
        out.push_str("  {\n",);
        let _ = writeln!(out, "    source = {},", lua_quote(&bin.source));
        let _ = writeln!(out, "    target = {},", lua_quote(&bin.target));
        out.push_str("    args = {",);
    ...
    out.push_str(
        "  if SYSTEM.ARCH == \"386\" or SYSTEM.ARCH == \"i386\" then return \
         \"32bit\" end\n",
    );
    ...
    out.push_str(
        "  for _, key in ipairs({ \"64bit\", \"32bit\", \"arm64\" }) do\n",
    );
    ...
    out.push_str(
        "    generated[name].windows = generated[name].windows or {}\n",
    );
    out.push_str("    for _, line in ipairs(lines) do\n",);
    out.push_str(
        "      table.insert(generated[name].windows, powershell_cmd(line))\n",
    );
    out.push_str("    end\n",);
    out.push_str("  end\n",);
    ...
    out.push_str("  else\n",);
    out.push_str(
        "    for i = 1, #dl.urls do table.insert(base_roots, \"source_\" .. \
         i) end\n",
    );
    out.push_str("  end\n",);
    out.push_str(
        "  for _, root in ipairs(base_roots) do table.insert(roots, root) \
         end\n",
    );
    ...
    out.push_str(
        "    if UTILS.FS.exists(candidate) then return candidate end\n",
    );
    ...
    out.push_str(
        "      if found and UTILS.FS.exists(found) then return found end\n",
    );
    out.push_str("    end\n",);
    out.push_str("  end\n",);
    out.push_str("  return nil\n",);
    ...
    let _ = writeln!(out, "  name = {},", lua_quote(&g.name));
    let _ = writeln!(out, "  repo = {},", lua_quote(&g.repo));
    let _ = writeln!(out, "  version = {},", lua_quote(&g.version));
    let _ = writeln!(out, "  description = {},", lua_quote(&g.description));
    let _ = writeln!(out, "  website = {},", lua_quote(&g.website));
    let _ = writeln!(out, "  license = {},", lua_quote(&g.license));
    let _ = writeln!(
        out,
    ...
    out.push_str(
        "  if not dl then error(\"No download source matched current \
         architecture\") end\n",
    );
    ...
    out.push_str(
        "    local source_path = resolve_bin_source(bin.source, roots)\n",
    );
    out.push_str("    if not source_path then\n",);
    out.push_str(
        "      error(\"Could not locate bin source in extracted files: \" .. \
         bin.source)\n",
    );
    out.push_str("    end\n",);
...
mod tests {
    ...
    fn parse_bin_string_and_array() {
        let bins = parse_bin_value(Some(&json!([
            "pwsh.exe",
            ["tools/foo.exe", "foo", "--x"]
        ]),),);
        ...
        assert_eq!(
            bins.first()
                .expect("should have at least one binary")
                .target,
            "pwsh"
        );
        assert_eq!(
            bins.get(1).expect("should have a second binary").target,
            "foo"
        );
        assert_eq!(
            bins.get(1).expect("should have a second binary").args,
            vec!["--x".to_string()]
        );
    }
    ...
    fn collect_download_specs_arch_fallback() {
        ...
        let specs =
            collect_download_specs(manifest.as_object().expect("object",),)
