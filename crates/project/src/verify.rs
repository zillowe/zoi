...
//!
//! This module provides functionality to verify that the local project
//! environment matches the state defined in `zoi.lock`. It ensures that all
//! installed packages are present, have the correct versions, and that their
//! file integrity (SHA-512) is maintained.
...
/// made that isn't reflected in the lockfile, this function will return an
/// error.
///
/// # Errors
///
...
pub fn run() -> Result<()> {
    ...
    for (pkg_key, lock_detail) in &lockfile_pkgs_map {
        if let Some(installed_pkg) = installed_pkgs_map.get(pkg_key) {
            if installed_pkg.version != lock_detail.version {
                return Err(anyhow!(
                    "Version mismatch for '{}': lockfile requires v{}, but \
                     v{} is installed.",
                    pkg_key,
                    lock_detail.version,
            ...
            if !version_dir.exists() {
                return Err(anyhow!(
                    "Package '{pkg_key}' is missing from the project's .zoi \
                     directory, though it is in the manifest."
                ));
            }
            ...
            use std::fmt::Write;
            let hex_key =
                pkg_key.as_bytes().iter().fold(String::new(), |mut acc, b| {
                    let _ = write!(acc, "{b:02x}");
                    acc
                });
            return Err(anyhow!(
                "Package '{pkg_key}' (hex: {hex_key}) from zoi.lock is not \
                 installed."
            ));
        }
    }
    ...
    for pkg_key in installed_pkgs_map.keys() {
        if !lockfile_pkgs_map.contains_key(pkg_key) {
            return Err(anyhow!(
                "Package '{pkg_key}' is installed in the project but is not \
                 in zoi.lock."
            ));
        }
    }
