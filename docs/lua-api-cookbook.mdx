...
> \[!TIP]
> Use the `.pkg.lua` extension for included files if you want them to be clearly associated with Zoi packaging, or `.lua` for general utility scripts. Note that `INCLUDE` uses paths relative to the current script.
