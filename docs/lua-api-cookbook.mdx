...
Use `ZOI.scope` to adjust installation paths or behaviors depending on whether the package is being installed for a single user, a project, or system-wide.

```lua
function package()
  if ZOI.scope == "system" then
    -- Install to a global location
    zcp("bin/tool", "${usrroot}/usr/bin/tool")
  else
    -- Standard store installation
...
- **zsh**: User data directory under `zoi/pkgs/shell/zsh/my-cli/_my-cli`
- **bash**: User data directory under `zoi/pkgs/shell/bash/my-cli/my-cli.bash`
- **fish**: User data directory under `zoi/pkgs/shell/fish/my-cli/my-cli.fish`
