...
3.  **Choose the Right Directory:**
    ...
    You can also create nested directories to better organize packages. For example, you could place a Linux-specific editor in `community/editors/linux/my-editor.pkg.lua`. The `repo` field in your package file should then be `community/editors/linux`.

4.  **Add Your Package File:**
    Copy your `my-package.pkg.lua` file into the `community/` directory.

    ```sh
    cp /path/to/my-package.pkg.lua community/
    ```
    ...
    cp /path/to/my-editor.pkg.lua community/editors/linux/
    ```
...
5.  **Commit and Push:**
    ...
    ```sh
    git add community/my-package.pkg.lua
    ...
    ```sh
    git add community/editors/linux/my-editor.pkg.lua
...
1.  **Generate Metadata:**
    Run `zoi package meta` on your `.pkg.lua` file. This creates a `.meta.json` file, resolving all dynamic Lua code into a static list of assets for all platforms.
...
2.  **Build the Archive:**
    Run `zoi package build` on the generated metadata file. This command downloads the assets for your **current platform**, verifies them, and creates a distributable `.pkg.tar.zst` archive.
...
2.  Create a `repo.yaml` file in the root of your repository to define its properties. See the [Repositories guide](/docs/zds/zoi/repositories#the-repoyaml-file) for details on the available fields. A minimal example would be:
    ...
    ```
3.  Structure the rest of your repository like the official `Zoi-Pkgs` database, with directories for tiers like `core`, `main`, `community`, etc.
