...
You can make a Git repository directly installable via `zoi install --repo <repo-spec>` by placing a `zoi.lua` file in its root. This file tells Zoi what package to install from your repository.

**Example `zoi.lua`:**

```lua
-- zoi.lua in the root of your repository
package("my-awesome-package")
```

The `package` declaration takes one source string, which can be:
