...
Zoi clones the repo into the user data directory under `zoi/pkgs/git/`. The name of the repository is determined from the URL (e.g. `my-zoi-repo`).
...
Place a root `zoi.lua` in a Git repository to make one or more package definitions installable with `zoi install --repo`.

```lua
package({
    main = "./packages/my-awesome-package",
    packages = {
        tools = "./tools.pkg.lua"
    }
})
...
`package("path")` is shorthand for `package({ main = "path" })`. Sources are repository-relative `.pkg.lua` files or folders. A folder must contain `<folder-name>.pkg.lua` or exactly one `.pkg.lua` file. Zoi clones the complete repository for dependency resolution and build execution.

```sh
# Install main from GitHub
zoi install --repo YourUsername/my-zoi-repo

# Select a named package export
zoi install --repo YourUsername/my-zoi-repo:tools

# Install from GitLab or Codeberg
zoi install --repo gl:YourUsername/my-zoi-repo:tools
zoi install --repo cb:YourUsername/my-zoi-repo:tools
```

A repository can import another repository with the same `zoi.lua` vocabulary:

```lua
imports({
    shared = { repo = "YourOrganization/shared-packages", rev = "main" }
})
