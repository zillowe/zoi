...
Zoi is also not as purely reproducible as Nix, although Zoi Specification v2 significantly closes the gap. Zoi improves reproducibility with `zoi.lock` (Spec v2), package hashes, registry metadata pinning, PGP verification, and project-local isolation, but external package managers can still change independently.
...
| Project environments          | `zoi.lua`, `zoi dev`, `zoi run`, `zoi env`                           | Shells and flakes                                   | Not usually the main model               | Runtime-focused           | Yes                             |
...
Zoi is the stronger choice if you want a packaging system that is easier to script with familiar Lua, works across Linux, macOS, and Windows, and can cooperate with existing package managers. Zoi v2 introduces Nix-like pinned inputs via its Lua project configuration and cryptographic state verification in its lockfiles.

Tradeoff:

- Nix gives stronger isolation and reproducibility via a purely functional approach.
- Zoi gives a more hybrid, host-aware workflow with scriptable Lua configuration and deep integration with native managers.

Choose Nix when you want the environment to be independent from the host. Choose Zoi when you want Nix-like pinning and isolation while maintaining first-class access to your host system's tools.
...
| Project install with `--frozen`                      | Highest Zoi project mode                                    |

For strict project installs, commit `zoi.yaml` and `zoi.lock`, then use:

```sh
zoi install --local --frozen
```
