...
If you wish to use one of the official Zoidberg mirrors (GitHub or Codeberg), you can change your active registry by setting it to the mirror's git URL:

```sh
# Set to GitHub mirror
zoi sync set https://github.com/zillowe/zoidberg.git

# Set to Codeberg mirror
zoi sync set https://codeberg.org/zillowe/zoidberg.git

# Revert to the built-in set registry
...
zoi pgp add https://zillowez.rf.gd/assets/zillowez.asc
```
...
These guidelines cover naming standards, versioning, and license requirements. Manual pages must be staged with `zman()` so they are available via `zoi man` (see the [Lua API reference](/docs/zds/zoi/lua-api#zman-source-section)).

## Build pipeline, deltas & retention

Each merged change to a `.pkg.lua` file is built per platform by the registry pipeline. For every platform build the pipeline publishes:

- the `.zpa` archive plus `.hash`, `.size`, `.files` and `.sig` sidecars,
- the `.zsa` source bundle plus its sidecars,
- a `.zdelta` patch against the previous published version plus `.hash`, `.size` and `.sig` sidecars (skipped when no previous version exists).

Deltas follow the `{name}-{version}-{platform}.zdelta` naming from `repo.yaml` and power bandwidth-efficient upgrades as described in [Repositories](/docs/zds/zoi/repositories#delta-patches).

Only the last three versions of each package are kept (plus the `latest` pointer). Older generic-package versions are pruned automatically on every platform.
