...
- **Storage Location**: Zoi's user data directory under `zoi/plugins/`
...
If you are inside a project with a `zoi.lua` file, this table is available:
...
Plugins can save and retrieve data that persists between different Zoi commands. The data is stored in the user data directory under `zoi/plugins/state.json`.
...
- **`zoi.env.get(name)`**: Returns the value of a plugin session environment variable override, or falls back to the process environment variable value. Returns `nil` if not set.
- **`zoi.env.set(name, value)`**: Sets a plugin session environment variable override. The override is available to the plugin runtime and commands executed via `zoi.sh`.
