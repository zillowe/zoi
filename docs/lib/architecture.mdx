...
For application integrations, prefer the stable wrappers in `crates/zoi-rs/src/lib.rs` over calling internal modules directly. The recommended public entry points are:
...
Zoi follows a structured, collision-resistant layout for managing package installations. User paths use XDG directories first, with platform-native fallbacks. See [Storage Locations](/docs/zds/zoi/storage).

### Store Root

All installed packages are stored in the `pkgs/store` directory within Zoi's data directory:

- **User Scope:** `$XDG_DATA_HOME/zoi/pkgs/store/`, or `~/.local/share/zoi/pkgs/store/` on Unix when unset.
- **System Scope:**
...
│   ├── manifest.json       # Installation manifest for this version
...
Binaries are not added to the system `PATH` individually. Instead, Zoi maintains a global shim directory per scope (`~/.local/bin` for user scope, `/usr/bin` for system scope on Linux) and creates symlinks to the binaries located inside the specific `{version}/bin/` directories in the store.
...
- **SQLite Indexing:** During `zoi sync`, Zoi automatically downloads these file lists and populates a dedicated `package_files` table in the SQLite database.
...
- **Key Storage:** the user data directory under `zoi/pgps/`
- **Format:** Public keys are stored as standard ASCII-armored `.asc` files.

### Built-in Keyring

Zoi features a built-in mechanism for embedding "Root of Trust" keys. During the build process, `build.rs` scans `src/builtin/pgp/` for `.asc` files and generates a static byte array containing their contents. On startup, Zoi's `ensure_builtin_keys()` function compares these embedded keys against the local store and automatically imports any that are missing or have been updated.
...
Transaction logs are stored as JSON files in the user state directory under `zoi/transactions/`. They are named using **UUID v7**, which provides both uniqueness and chronological ordering.
  ...
  "operations": [
    {
      "install": {
        "manifest": {
          ...
          "scope": "user",
          "installed_files": ["/home/user/.local/share/zoi/pkgs/store/.../1.0.0/bin/hello"]
        }
      }
    }
  ]
...
- **Failure Rollback:** If an install, uninstall, or update fails after a transaction starts, Zoi reads the current transaction log and reverts recorded operations in **reverse order**.
- **Just-in-Time Escalation:** For system-wide operations, Zoi maintains user-space atomicity by building/resolving first, and only escalating to root via a helper subcommand for the final destructive phase.
- **Manual Transaction Rollback:** Users can revert all operations in a transaction using `zoi transaction undo [id]` (defaults to the most recent transaction).
