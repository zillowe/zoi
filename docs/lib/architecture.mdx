...
Binaries are not added to the system `PATH` individually. Instead, Zoi maintains a global `bin` directory under its user data directory (`zoi/pkgs/bin`) and creates symlinks to the binaries located inside the specific `{version}/bin/` directories in the store.
