...
This example will cause Zoi to find the source code for `git`, download it, and run the build commands specified in its `.pkg.lua` file, even if a pre-compiled binary is available.
