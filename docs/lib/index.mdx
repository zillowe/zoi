...
When `frozen` is `true`, `install_sources` follows the exact entries recorded in `zoi.lock` rather than re-resolving the project from `zoi.lua`. That preserves pinned versions, selected sub-packages, and recorded dependency choices for reproducible project installs.
