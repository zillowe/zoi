...
- `ZOI.PATH.user`: Path to Zoi's user data directory (`$XDG_DATA_HOME/zoi`, with a platform-native fallback).
...
| Variable           | Type     | Description                                                     |
| :----------------- | :------- | :-------------------------------------------------------------- |
| `ZOI.PKG.store`    | `string` | Absolute path of the package store in Zoi's user data directory. |
...
Stages a shell completion file for a specific shell. The file is copied to `${pkgstore}/shell/{shell}/{filename}` and symlinked into the global completions directory in the user data directory. Zoi automatically manages the symlinks during install and uninstall.
