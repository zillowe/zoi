...
pkgs:
  - eza
  - fzf: "0.44.1" # Version map support
...
commands:
  ...
  - cmd: test
    run: npm test
    # Feature: Task dependencies (DAG)
    depends_on: ["build"]

  - cmd: build
    run: npm run build
    # Feature: File-based caching
    cache_files: ["src/**", "package-lock.json"]
  ...
  - name: Web development environment
    ...
    run:
      ...
      windows-amd64:
        ...
        - npm run build:win
    env:
      # Platform-specific environment maps use platform keys directly.
      # `default` is used when the current platform has no exact match.
      default:
        NODE_ENV: "production"
      windows-amd64:
...
# Optional: Shell configuration for `zoi dev`
shell:
  env:
    # Custom environment variables for the dev shell
    DEBUG: "true"
```

For platform-specific `env`, use platform keys at the top level:
...
* `pkgs`: `list` of strings or objects (optional). A list of packages to be installed locally to the project. Supports version maps (e.g. `- fzf: "0.44.1"`). Requires `config.local: true`.
...
* `commands`: `list` of objects (optional)
  ...
  * `env`: `map` (optional)
    ...
    * Can be a platform map where keys are platform strings such as `linux-amd64`, plus optional `default`, and values are environment-variable maps.
  * `depends_on`: `list` of `string` (optional). List of other command aliases that must run successfully before this command.
  * `cache_files`: `list` of `string` (optional). List of file glob patterns. The command will be skipped if the hashes of these files haven't changed since the last successful run.
...
* `shell`: `object` (optional)
  * `env`: `map` (optional) - Environment variables to set when entering `zoi dev`. Same structure as in `commands`.
...
3. Create two files:
   * `.zoi/pkgs/zoi.pkgs.json`: A detailed manifest of all installed packages, their dependencies, and metadata. This file is intended for local tracking.
   * `zoi.lock`: A file containing cryptographic hashes of the installed packages in the project root. This file is used to verify the integrity of the packages and should be committed to your repository.
...
```sh
zoi install --frozen-lockfile --local
```

This installs only versions pinned in `zoi.lock` and refuses to update the lockfile during the run.
...
* Registry handle keys (e.g. `#zoidberg`): A flattened map containing detailed information about each package, including its version, integrity hash, dependencies, and any chosen optional dependencies.
...
* Run a command directly in the dev shell environment and exit:

```sh
zoi dev --run "make build"
```
