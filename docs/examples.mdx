...
<Callout title="Metadata">
  ...
  fields described in [Creating Packages](/docs/zds/zoi/creating-packages),
  including `name`, `repo`, `description`, `maintainer`, and either `version` or
  `versions`.
</Callout>
...
## 6. Project Environment (`zoi.yaml` and `zoi.lua`)

Zoi supports two formats for project configuration: `zoi.yaml` (declarative) and `zoi.lua` (scriptable).

### `zoi.yaml` (Declarative)
...
Ensure user modifications to configuration files are preserved during upgrades. The `backup` field supports path placeholders like `${usrroot}` and `${usrhome}`.
...
metadata({
  ...
  -- Supports placeholders for system-wide or user-specific paths
  backup = { "${usrroot}/etc/server.conf", "etc/relative_to_pkgstore.conf" },
  types = { "pre-compiled" }
})
...
function uninstall()
  ...
  -- Remove a system-wide log directory (Zoi handles sudo escalation automatically)
...
## 29. Shell Completions (`zshell`)

Bundle shell completion scripts with your package. Zoi automatically symlinks them into the global completions directory and removes them on uninstall.
...
metadata({
  ...
  maintainer = { name = "Author", email = "author@example.com" },
  bins = { "my-tool" },
  types = { "pre-compiled" },
  license = "MIT",
})
...
function prepare()
  local url = string.format(
    "https://example.com/my-tool-%s-%s.tar.gz",
...
function package()
  zcp("src/my-tool", "${pkgstore}/bin/my-tool")

  -- Bundle shell completions for multiple shells
  zshell("${pkgluadir}/completions/_my-tool", "zsh")
  zshell("${pkgluadir}/completions/my-tool.bash", "bash")
  zshell("${pkgluadir}/completions/my-tool.fish", "fish")
end
```

After installation, completions are symlinked to:

```
~/.local/share/zoi/pkgs/shell/zsh/my-tool/_my-tool
~/.local/share/zoi/pkgs/shell/bash/my-tool/my-tool.bash
~/.local/share/zoi/pkgs/shell/fish/my-tool/my-tool.fish
```

For **zsh**, add the parent directory to your `fpath` in `.zshrc`:

```bash
fpath=(~/.local/share/zoi/pkgs/shell/zsh $fpath)
autoload -Uz compinit && compinit
```

For **bash** and **fish**, completions are available automatically if the global shell directory is on your completion search path.
