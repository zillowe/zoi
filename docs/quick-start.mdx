...
Zoi links package binaries into `~/.local/bin` on Linux and other Unix systems. If commands installed by Zoi are not found, configure your shell completions and PATH. See [Storage Locations](/docs/zds/zoi/storage) for the exact platform path:
...
Zoi Spec v2 uses `zoi.lua` for tools and tasks (the previous `zoi.yaml` is only for tasks now).
...
| Source bundle        | `zoi install ./my-tool.zsa`           |
| Pre-built archive    | `zoi install ./my-tool-1.0.0.zpa`     |
...
<Callout title="Persistent Shell">
  The `cmd()` function uses a **persistent shell session**. Directory changes
  (`cd`) and exported variables persist between separate `cmd()` calls within
  the same function.
</Callout>
...
| Problem                                     | Fix                                                                             |
| ------------------------------------------- | ------------------------------------------------------------------------------- |
| Installed commands are not found            | Run `zoi shell <shell>` or add Zoi's user bin directory to `PATH`.              |
| Package cannot be found                     | Run `zoi sync`, then check active repos with `zoi repo list`.                   |
| You need a mirror                           | Set the registry to a mirror's git URL via `zoi sync set` (e.g. `https://github.com/zillowe/zoidberg.git`). |
