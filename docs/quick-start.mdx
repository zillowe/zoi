...
A Zoi project is a single `zoi.lua` file covering tools, tasks, and environments.
...
tasks({
    { cmd = "build", run = "make build" },
    { cmd = "test", run = "make test" },
})
```
...
Commit `zoi.lua` and `zoi.lock` so other machines can reproduce the same environment.
...
- Read [Project Configuration](/docs/zds/zoi/project-config) for `zoi.lua`.
