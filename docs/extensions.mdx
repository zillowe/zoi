...
- **Effect:** A new `zoi.yaml` file is created in the current directory where `zoi extension add` is run. The command will fail if a `zoi.yaml` file already exists there.
- **On Removal:** The `zoi.yaml` file created during installation is deleted from that original target directory.
