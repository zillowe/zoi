...
Zoi extensions are a special type of package that, instead of installing software, modify Zoi's own configuration or project setup. They provide a powerful way to distribute and manage collections of settings, such as custom package repositories, or even entire project starter templates (`zoi.lua`) as a single, versionable package.
...
- **Project Starters:** Distribute a standard `zoi.lua` file for a specific type of project (e.g. Rust library, web service) to bootstrap development environments quickly.
...
- **Effect:** Zoi clones the repository into the user data directory under `zoi/pkgs/git/`. Users can then install packages from it using the `#git@<repo-name>/<pkg-name>` syntax.
...
- **On Removal:** The previous default registry URL is restored.
...
Creates a `zoi.lua` file in the current directory, allowing for sharable project configurations.

- **Purpose:** To distribute a standard `zoi.lua` project file that can be used to set up a development environment.
- **`add` Value:** The raw, multi-line string content of the `zoi.lua` file.
- **Effect:** A new `zoi.lua` file is created in the current directory where `zoi extension add` is run. The command will fail if a `zoi.lua` file already exists there.
- **On Removal:** The `zoi.lua` file created during installation is deleted from that original target directory.
...
- **Effect:** Zoi downloads or fetches the key and adds it to the user data directory under `zoi/pgps/`.
- **On Removal:** The PGP key file is deleted from the store.

#### `plugin`

Distributes a Lua script that extends Zoi's core CLI functionality with custom subcommands.

- **Purpose:** To add new logic, workflows, or integrations to Zoi.
- **Fields:**
  - `name`: A unique name for the plugin.
  - `script`: The raw Lua script content.
- **Effect:** Zoi saves the script to the user data directory under `zoi/plugins/` and loads it on startup. The script can use the `zoi` API to register commands.
- **On Removal:** The plugin script is deleted.

#### `hook`

Distributes a global transaction hook.

- **Purpose:** To perform system-wide maintenance tasks (e.g. updating font caches) automatically when relevant files are modified.
- **Fields:**
  - `name`: A unique name for the hook.
  - `content`: The raw YAML content of the hook definition.
- **Effect:** Zoi saves the hook to the user data directory under `zoi/hooks/<name>.hook.yaml`. It will be triggered during future transactions based on its path matching logic.
- **On Removal:** The hook file is deleted.
...
This example shows how to create a "project starter" that provides a ready-to-use `zoi.lua` for a new Rust project.
...
metadata({
  ...
  type = "extension",
  version = "1.0",
  description = "A starter zoi.lua for a Rust project.",
  maintainer = {
    name = "Your Name",
    email = "your.email@example.com"
  },
...
packages({
    "@core/rust",
    "@community/cargo-watch",
})
checks({
    { name = "rust", check = "rustc --version" },
    { name = "cargo-watch", check = "cargo watch --version" },
})
tasks({
    { cmd = "run", run = "cargo run" },
    { cmd = "test", run = "cargo test" },
    { cmd = "watch", run = "cargo watch -x run" },
})
]]
      }
    }
...
A user can now navigate to an empty project directory, run `zoi extension add my-rust-starter`, and immediately have a working `zoi.lua` to start developing.

### Custom Subcommand Plugin

This example shows how to create a plugin that adds a new `zoi hello` command.

```lua
-- hello-plugin.pkg.lua
metadata({
  name = "hello-plugin",
  repo = "community",
  type = "extension",
  version = "1.0",
  description = "Adds a custom 'hello' command to Zoi.",
  maintainer = {
    name = "Your Name",
    email = "your.email@example.com"
  },
    ...
    changes = {
      {
        ...
        name = "hello-cmd",
        script = [[
zoi.register_command("hello", function(args)
    print("Hello from Zoi Plugin!")
    print("Your OS is: " .. zoi.system.os)
    if #args > 0 then
        print("Arguments passed: " .. table.concat(args, ", "))
    end
end)
]]
      }
...
After adding this extension with `zoi extension add hello-plugin`, you can run `zoi hello world` and see the plugin's output.
