...
* **Effect:** Zoi saves the script to `~/.zoi/plugins/` and loads it on startup. The script can use the `zoi` API to register commands.
...
* **Effect:** Zoi saves the hook to `~/.zoi/hooks/<name>.hook.yaml`. It will be triggered during future transactions based on its path matching logic.
