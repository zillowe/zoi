...
- **Fields:**
  ...
  - `authorities`: (list of string, optional) PGP key identifiers trusted as authorities for package signatures from this registry.
- **Example:**
...
- **Type:** `object`
- **Fields:**
  - `enable`: (boolean) If `true`, Zoi will _only_ install pre-built packages (`.pkg.tar.zst` archives) that are signed by a trusted PGP key. It will not fall back to building from source.
