...
A list of additional directories to search for `.pkg.tar.zst` archives during installation.
...
- `parallel_jobs_unoverridable`: If `true`, the `parallel_jobs` setting is locked.
...
- **Type:** `object`
- **Fields:**
  - `enable`: (boolean) If `true`, Zoi will _only_ install pre-built packages (`.pkg.tar.zst` archives) that are signed by a trusted PGP key. It will fail on unsigned packages and will not fall back to building from source.
