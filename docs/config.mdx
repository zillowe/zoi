...
2. **User:** User-specific configuration. This is the most common file for users to edit.
   - **Linux and other Unix:** `$XDG_CONFIG_HOME/zoi/config.yaml`, or `~/.config/zoi/config.yaml` when unset.
   - **macOS and Windows:** `$XDG_CONFIG_HOME/zoi/config.yaml` when set, otherwise the platform-native configuration directory under `zoi`.
...
Configures the primary package database (registry) that Zoi syncs with. The `handle` may refer to a built-in registry definition (see [Specifications](/docs/zds/zoi/specs#1-built-in-registry-definition)); when it does, Zoi auto-populates the `name`, `description`, and `url` fields from that definition when you run `zoi sync set <handle>`.

- **Type:** `object`
- **Fields:**
  - `handle`: (string) A short, unique name for the registry. Zoi will determine this automatically on the first sync if left empty.
  - `url`: (string) The Git URL of the registry repository. Left for Zoi to derive when the `handle` matches a built-in registry.
  - `name`: (string, optional) Display name. Populated automatically for built-in registries.
  - `description`: (string, optional) Short description. Populated automatically for built-in registries.
...
A list of supplementary package registries to sync with, in addition to the `default_registry`. As with `default_registry`, a `handle` that matches a built-in definition auto-populates the metadata when you run `zoi sync add <handle>`.
...
A list of raw Git repository URLs that contain Zoi packages. These are cloned into the user data directory under `zoi/pkgs/git/` and can be used for personal or third-party package collections that are not full registries.
...
Enables or disables persistent audit logging of all package operations (install, uninstall, upgrade). Logs are stored in Zoi's user state directory under `zoi/audit/audit.json` and can be viewed with `zoi history`.
...
If `true`, Zoi will make the package database directory in the user data directory under `zoi/pkgs/db` read-only after a `zoi sync` operation. Before syncing, it will automatically make it writable. This is a security feature to prevent accidental or malicious modification of package definitions.
...
- **Type:** `integer`
- **Default:** `4`
- **Value of 0:** Infinite generations (no pruning).
- **Note:** The current active generation is never pruned.
- **Example:** `system_generations_limit: 10`

### `max_delta_steps`

Defines the maximum number of sequential `.zdelta` patches Zoi will download and apply when upgrading a package. When the cached version is more than one release behind the target, Zoi tries each intermediate delta in order (for example `149 -> 150`, then `150 -> 151`) instead of downloading the full archive. If the upgrade spans more versions than this limit, or any delta step fails, Zoi falls back to downloading the full archive.

- **Type:** `integer`
- **Default:** `3`
- **Value of 0:** Disables delta upgrades (always downloads the full archive).
- **Example:** `max_delta_steps: 5`
...
- `system_generations_limit_unoverridable`: If `true`, the `system_generations_limit` setting is locked.
- `max_delta_steps_unoverridable`: If `true`, the `max_delta_steps` setting is locked.
