...
- **Efficient Self-Updates:** Uses patch-based updates (`bsdiff`) for smaller and faster upgrades.
- **Advanced CLI Tools:**
...
- **SBOM & Reproducible Installs:** Automatically records installed packages to an SBOM (`zoi.pkgs.json`) and generates sharable manifest files for reproducible dependency choices.
...
- **Interactive Creation:** Use `zoi make` to interactively create a package definition file.
