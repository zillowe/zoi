...
- **Persistent Shell Scripting:** The `cmd()` function maintains state (like `cd` and `env`) between calls, providing a natural scripting experience.
...
- **Project Environments:** Manage project tools with `zoi.lua` and named tasks with `zoi.yaml`.
