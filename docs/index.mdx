...
- **Project Environments:** Manage project-specific commands and environments using a local `zoi.lua` file.
...
- **Advanced CLI Tools:**
  ...
  - `zoi transaction undo`: Revert all operations recorded in a transaction.
...
zoi transaction undo              # Undo if broken
```
...
<Accordions type="single">
  ...
  <Accordion title="What is 'zoi dev' and how does it differ from 'zoi env'?">
    ...
    ensures all packages in your `zoi.lua` are installed locally and sets up
  ...
  <Accordion title="Where are binaries installed and how do I fix PATH?">
    Binaries are placed in the user data directory and linked into its
    `zoi/pkgs/bin` directory. Zoi attempts to add this to your shell PATH on Unix and user PATH on Windows.
    If commands are not found, run `zoi shell <shell>` to configure your shell. See [Storage Locations](/docs/zds/zoi/storage) for the exact platform path.
  </Accordion>
  ...
  <Accordion title="How can I back up and restore my installed packages?">
    ...
    in Zoi's user state directory under `zoi/pkgs/zoi.lock`. You can back up this file. To restore all your
