...
| `install`    | Installs one or more packages. Can also install from a `zoi.pkgs.json` file (to restore all packages) or a `<pkg>.manifest.yaml` file (for reproducible dependency choices). <br/>`--force`: Force re-installation if the package already exists. <br/>`--interactive`: Choose the installation method interactively. <br/>`--all-optional`: Install all optional dependencies. |
...
# (Optional) A URL to a markdown or text manual file for the `zoi man` command.
man: https://my-awesome-app.com/MAN.md
...
scope: user
# (Optional) The package type. Can be 'package' (default), 'collection', 'service', 'config', 'app', 'extension', 'library', or 'script'.
type: package
...
# (Optional) For 'library' type packages, provides pkg-config information.
pkg_config:
  description: "My Awesome Library"
  libs: "-L/usr/local/lib -lmy-awesome-app"
  cflags: "-I/usr/local/include/my-awesome-app"
...
<Accordions type="single">
  <Accordion title="How do I create my own package for Zoi?">
    You can create a `pkg.yaml` file that defines your package. The easiest way is to run `zoi make` for an interactive setup. This file includes metadata like the package name, version, description, and installation instructions. The `Package` struct in [`types.rs`](https://gitlab.com/Zillowe/Zillwen/Zusty/Zoi/-/blob/main/src/pkg/types.rs) shows all available fields, and there's a [schema](https://gitlab.com/Zillowe/Zillwen/Zusty/Zoi/-/blob/main/app/pkg.schema.json) you can use in your `pkg.yaml` file. For more info about creating and publishing a package please visit [this guide](/docs/zds/zoi/creating-packages).
  </Accordion>
</Accordions>
<br />
<Accordions type="single">
