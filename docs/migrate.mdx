...
| :----------------- | :-------------------------- | :------------------------------------------- |
| **Project Config** | `zoi.yaml`                  | `zoi.lua` (primary), `zoi.yaml` (tasks only) |
...
### 1. Migrating `zoi.yaml` to `zoi.lua`

In Specification v2, `zoi.lua` is the primary scriptable configuration for your project. The `zoi.yaml` file is now restricted strictly to task definitions (`run` and `env`).

**Action Required:**
Move your `pkgs`, `registries`, and `config` fields from `zoi.yaml` to a new `zoi.lua` file.

**Zoi v1 / Spec v1 (`zoi.yaml`):**
