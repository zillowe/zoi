...
Implementing a `test()` lifecycle function is **highly recommended** for the `core` and `main` tiers, and strongly encouraged for all others. The test should verify that the binary executes correctly (e.g. running `myapp --version`). Zoi supports installing `test` dependencies defined in the `dependencies()` block when the user or CI runner specifies `--install-deps` with `zoi package test` or `zoi package build --test`.
...
Use the `zman()` helper function to bundle manual pages. This ensures they are available via `zoi man` and placed in standard locations.
