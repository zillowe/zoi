...
- **`PKG.scope`**: Define a default `scope` if your package is intended for a specific target (e.g. `system` for core libraries).
- **`PKG.scopes`**: Use the `scopes` list to restrict where a package *can* be installed. This is mandatory for core system components that would be non-functional in `user` scope.
