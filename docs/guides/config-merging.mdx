...
The `backup` field supports placeholders to reference absolute system paths or user-specific locations:

- `${usrroot}`: The system's root directory (e.g. `/etc/my-app.conf`).
- `${usrhome}`: The user's home directory (e.g. `~/.config/my-app.conf`).
- `${pkgstore}`: The package's own version directory.
- `${applications}`: The system or user applications directory.
