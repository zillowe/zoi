...
1. **Transaction Begin**: Zoi generates a UUID v7 transaction ID and creates a temporary JSON log in `~/.zoi/transactions/`.
2. **Recording Operations**: Package install, uninstall, and upgrade operations are recorded with their manifests, including the files listed in those manifests.
3. **Atomic Commits**: If all steps in the operation succeed, Zoi commits by deleting the transaction log.
4. **Rollback on Failure**: If an install, uninstall, or update fails after a transaction starts, Zoi attempts to revert the operations recorded in the current transaction.

### The Last Transaction

You can manually revert the newest remaining transaction log using the `--last-transaction` flag:
