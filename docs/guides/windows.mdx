...
- **Unix (Linux/macOS)**: `cmd()` executes commands via a **persistent `bash` session**.
- **Windows**: `cmd()` executes commands via a **persistent `pwsh` session** (PowerShell Core).

### State Persistence

Because the shell session is persistent, directory changes and variable assignments stick:

```lua
-- On Windows, this works across calls:
cmd("$env:MY_VAR = 'hello'")
cmd("Write-Host $env:MY_VAR") -- Prints 'hello'
```
...
**Prefer (Portable):**
Zoi provides the `SYSTEM` and `ZOI` tables to help you handle logic in Lua instead of the shell. Additionally, Zoi automatically exports common metadata as environment variables in both shell types.
...
When setting environment variables for a command, use the `zoi.lua` `env` mapping for project-level tasks, or set them directly in Lua if building a package.
