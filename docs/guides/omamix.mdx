...
You will add your custom `.pkg.lua` files to this repository.

## Step 2: Define Your Core Tools with a Collection

First, let's define the list of software you want to install.

Create a file in your new repository named `dev-environment.pkg.lua`:

```lua
-- dev-environment.pkg.lua
package({
...
Create a file named `my-dotfiles.pkg.lua` in your setup repository:
...
package({
  ...
  dependencies = {
    runtime = {
      required = { "native:starship", "native:zsh", "native:neovim" }
    }
  },
  config = {
    {
...
Now, update your `dev-environment.pkg.lua` to include your new dotfiles config as a dependency:
