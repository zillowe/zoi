...
publish-pkg:
 ...
 echo "$HOME/.local/bin" >> $GITHUB_PATH
    ...
    - name: Build Package
      run: zoi package build my-tool.pkg.lua --type source --platform linux-amd64
```
...
provision-env:
  ...
  script:
    - zoi sync
    - zoi install --save # Installs everything listed in your project's zoi.lua
  artifacts:
    paths:
      - .zoi/pkgs/store/
...
run-tests:
  ...
  dependencies:
    - provision-env
  script:
    - zoi env test # Runs the 'test' environment defined in zoi.lua
```
...
- name: Cache Zoi Store
  ...
  with:
    path: ~/.local/share/zoi/pkgs/store
    key: ${{ runner.os }}-zoi-${{ hashFiles('zoi.lua', 'zoi.lock') }}
    restore-keys: |
      ${{ runner.os }}-zoi-
```
...
3. **Robust Metadata Extraction**: When you need to extract package information (like name, version, or repo) in your scripts, use `zoi package inspect --json <file>`. This is much more reliable than using `grep` or `sed` on raw Lua files.

   ```bash
   # Example: Get package name using jq
   PKG_NAME=$(zoi package inspect --json my-pkg.pkg.lua | jq -r '.name')
   ```

4. **Specific Platforms**: When building packages with `zoi package build`, explicitly specify the `--platform` to ensure you are creating the correct artifacts for your target users.
5. **Local Project Scopes**: For CI jobs that only need tools for the current build, use `--scope project` or `--local` to keep the installation isolated to the project directory.
