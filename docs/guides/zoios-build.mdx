...
4. **A Configured Registry**: Your Zoi instance must be configured to sync with a registry that provides the essential ZoiOS packages (e.g. `@parlex/linux`, `@parlex/glibc`, `@parlex/systemd`).
...
2. **Root Partition** (e.g. `/dev/sdX2`, remainder of disk, format as `ext4`)

```bash
# Example formatting
sudo mkfs.vfat -F 32 -n ZOI_BOOT /dev/sdX1
sudo mkfs.ext4 -L ZOIOS_ROOT /dev/sdX2
```
...
2. **Bootstrap Installation**: Directly installs the entire base package set (`@parlex/linux`, `@parlex/glibc`, etc.) into the target Zoi store and then into the system root.
...
dracut --kver 7.1.4-arch1-1 --force /boot/initramfs-linux.img
```
...
### 3. Post-Build Package Management
...
sudo zoi install @parlex/neovim @parlex/git --root /mnt
```

<Callout title="Isolated State">
  When using `--root`, Zoi is fully isolated. System package data is stored within the target's `/var/lib/zoi`; user state follows the target user's XDG or platform-native directories. By default, metadata is synced to the system-wide store (`/var/lib/zoi/pkgs/db`), ensuring the new OS is ready for use even without a pre-configured user home.
</Callout>
