...
You need the actual package binaries (`.zpa`). You can download these on a machine with internet and move them to the air-gapped system.
...
```sh
zoi cache add ./my-downloads/*.zpa
```
...
You can bundle an entire Zoi registry (including the Git history and the SQLite `.db` files) into a compressed archive and extract it into the user data directory under `zoi/pkgs/db/` on the target machine.
...
- `zoi cache clear`: Remove everything from the local cache, including archives and cached package definitions.
