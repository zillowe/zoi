...
| `trigger.dirs`      | `list`   | A list of directory paths. If any modified file is inside one of these directories, the hook triggers. Accepts `${usrroot}`, `${usrhome}`, `${pkgstore}`, `${applications}` placeholders as well as absolute (`/usr/...`) and relative (`usr/...`) forms. |
| `trigger.paths`     | `list`   | A list of glob patterns. If any modified file matches, the hook triggers. Also supports Arch-style directory paths ending in `/`. Accepts the same placeholders and path forms as `trigger.dirs` (e.g. `${usrroot}/usr/share/fonts/**`). |
| `trigger.packages`  | `list`   | A list of package names. If any of these packages are installed, upgraded, or removed, the hook triggers. |
