...
- **Delta Upgrades:** When a previous version of an archive is cached, upgrades automatically try to fetch and apply a `.zdelta` (bsdiff-based) instead of re-downloading the full archive. Delta patches are explicitly configured in the registry's `repo.yaml` (delta section) with their own URLs, hash files, and PGP signatures. The delta patch is verified for integrity and authenticity before application, falling back silently to a full download whenever a delta is unavailable or fails verification.
