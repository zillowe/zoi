...
- **Scope-Aware Registry Sync:** Synchronize package metadata to either your user directory (`~/.zoi`) or the system-wide store (`/var/lib/zoi`). Zoi automatically detects the correct scope when running as root or with a sysroot (`--root`).
...
- **Local Package Development:** A dedicated `zoi package` command set streamlines the creation and testing of packages locally:
  - `zoi package build`: Build a distributable package archive (`.zpa`). Supports `--pure` for isolated builds using a clean sysroot.
