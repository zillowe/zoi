...
* **Zoi Mini (Lightweight Service):** A minimal version of Zoi that operates without a local database, resolving package metadata and vulnerabilities on the fly from the Zoidberg registry. Perfect for one-liner installations via `zm.sh` or `zm.ps1`.
