...
* **Reproducible Project Installs (`--frozen-lockfile`):** Rebuild a project environment from `zoi.lock` without re-resolving versions, sub-packages, or recorded dependency choices.
...
* **Powerful CLI Tools:** Zoi offers a rich set of commands for package management and inspection:
  ...
  * `zoi dev`: Enter a development shell for the current project, with environment variables (PATH, LD\_LIBRARY\_PATH, CPATH, etc.) automatically set up based on dependencies.
...
* `-f`, `--files`: Download and index file lists for global search (requires server-side support).
