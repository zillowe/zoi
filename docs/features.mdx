...
- **Cross-Platform:** Works natively on Linux, macOS, and Windows, supporting `amd64` and `arm64` architectures. Write one package definition and let Zoi handle the platform differences.
  - **macOS Integration:** Automatically handles macOS `.app` bundles, stripping Gatekeeper quarantine flags and symlinking them to `~/Applications` or `/Applications`.

- **Just-in-Time Privilege Escalation:** Automatically prompts for `sudo` password only when system-wide operations (like extracting to `/var/lib/zoi` or linking to `/usr/local/bin`) are required. This ensures your local package database and caches remain in your user-space, avoiding the need to run the entire CLI as root.

- **ZoiOS Management (Exclusive):** Manage an entire operating system state declaratively via `/etc/zoi/system.lua`.
  - **Transaction-Linked Generations:** Every system change creates a versioned "Generation" record linked to a Zoi package transaction. Revert any OS update safely by rolling back the associated transactions.
  - **Direct FHS Installation:** ZoiOS installs packages directly into the system root (`/usr/bin`, `/usr/lib`, etc.), ensuring 100% compatibility with standard Linux software while maintaining a versioned store for safety.
  - **Universal Distro Builder:** Bootstrap an entire ZoiOS distribution onto a manually partitioned and formatted disk or directory with a single command (`zoi system distro build`).
  - **Native Distro Chroot:** Enter any ZoiOS root directory with `zoi system distro chroot`, which automatically sets up a secure sandbox with `/dev`, `/proc`, and `/sys` mounted from the host.

- **Explicit Scope Control:** Restrict package installation to specific scopes (`user`, `system`, or `project`) using the `scopes` metadata field. Zoi automatically enforces these constraints during installation and updates.

- **Scope-Aware Registry Sync:** Synchronize package metadata to either the user data directory or the system-wide store (`/var/lib/zoi` on Linux). Zoi automatically detects the correct scope when running as root or with a sysroot (`--root`).
...
- **Decentralized Security Advisories:** Automated vulnerability tracking via `.sec.yaml` files co-located with packages in the registry. Zoi proactively warns you during installation or upgrades if a package has known security issues, requiring explicit confirmation to proceed.
...
- **Automated Relocation Engine:** Automatically rewrites ELF binary headers (`RUNPATH`/`RPATH`) to use `$ORIGIN` relative paths during build. This ensures Zoi packages are fully relocatable and can find their internal shared libraries regardless of where they are installed or symlinked.

- **Lua-Based Project and Repository Configuration (`zoi.lua`):** Define project environments with a scriptable Lua API and export one or more package definitions from the same repository. The manifest supports project metadata, packages, tasks, environments, pinned repository imports, and selectable package exports.

- **Hermetic Project Environments:** Strictly isolate project dependencies and registries within `./.zoi/pkgs/`. Ensure that project-level syncs do not affect global state and remain reproducible across machines.

- **Pinned Registry and Import Inputs:** Lock registries and `imports()` repositories to exact Git revisions in `zoi.lua`. `zoi.lock` records the requested ref, resolved commit, manifest hash, and exported package paths.

- **Reproducible Project Installs (`--frozen`):** Rebuild a project from `zoi.lock` using pinned registry and import commits, manifest/config hashes, source provenance, exact versions, selected options, optionals, and dependency selections. Directory and package hashes verify the resulting local state.

- **Automated Dev Repo Discovery (`zoi dev --repo`):** Instantly enter a development shell for any Git repository. Zoi clones the repository to a temporary directory, sets up the environment defined in its `zoi.lua`, and cleans up automatically on exit.

- **Global Transaction Hooks:** Automate system maintenance tasks (like font cache updates) based on modified file paths or package names. Hooks run once per transaction, preventing redundant script execution. Support for sysroots ensures hooks execute correctly inside the target OS during distribution builds.

- **Transaction Inspection:** Inspect recorded package operations and touched files with `zoi transaction list`, `zoi transaction show`, and `zoi transaction files`.

- **Delta Upgrades:** When a previous version of an archive is cached, upgrades automatically try to fetch and apply a `.zdelta` (bsdiff-based) instead of re-downloading the full archive. When the cached version is several releases behind, Zoi applies each intermediate delta in sequence until the target version is reached, limited by the `max_delta_steps` config (default `3`). Delta patches are explicitly configured in the registry's `repo.yaml` (delta section) with their own URLs, hash files, and PGP signatures. Each delta patch is verified for integrity and authenticity before application, falling back silently to a full download whenever a delta is unavailable or fails verification.

- **Embedded Signatures:** Packages can be signed so the signature travels inside the archive itself (`manifest.sig`), in addition to the legacy detached `.sig` sidecar which remains fully supported. Delta patches can also be signed and verified.
...
- **Version Multiplexing via Intelligent Shims:** Automatically switch between different versions of the same tool (e.g. Node.js 18 vs 20) based on your current project (`zoi.lua`), global configuration, or legacy files (like `.nvmrc` and `.python-version`) via its extensible Lua plugin system. Replaces static symlinks with intelligent shims that resolve the correct version at runtime.

- **Shell Completions via `zshell()`**: Bundle shell completion scripts (Bash, Zsh, Fish, Elvish) with your packages using the `zshell()` lifecycle function. Zoi automatically symlinks completions into per-package directories under the user data directory and cleans them up on uninstall. Supports versioned completions since each package version's files are stored separately in the Zoi store.

- **Manual Pages via `zman()`**: Bundle manual pages with your packages using the `zman()` lifecycle function. Zoi automatically places them in standard locations (e.g. `/usr/share/man/` for system scope) and supports viewing them via `zoi man`, even for uninstalled packages.

- **Generic Project Handlers via Plugins:** Extend `zoi install` to support any ecosystem (NPM, Pip, etc.) by implementing simple Lua hooks that detect project files and manage dependencies using native high-performance APIs.
...
- **Stateful Shell Scripting**: The `cmd()` function uses a persistent shell process, allowing directory changes (`cd`) and environment variable exports to persist between calls. This makes writing package scripts feel natural and less boilerplate-heavy.
...
- **Project-Level Environments:** Use a `zoi.lua` file in your project root to define custom commands and development environments. Manage project-specific tasks, tools, packages, and environment variables with `zoi run`, `zoi env`, and `zoi dev`, [learn more](/docs/zds/zoi/project-config).
...
- **Secure and Verifiable:** Ensure the integrity and authenticity of your packages:
  - **Checksums:** Verify downloads against `SHA512` or `SHA256` hashes.
...
- **Local Package Development:** A dedicated `zoi package` command set streamlines the creation and testing of packages locally:
  - `zoi package build`: Build a distributable package archive (`.zpa`). Automatically bundles the package into a source archive (`.zsa`) first and builds from it; use `--no-zsa` to build directly from the `.pkg.lua` file. Supports `--pure` for isolated builds using a clean sysroot.
  - `zoi package bundle`: Bundle a package and its local assets into a source archive (`.zsa`).
  - `zoi package install`: Install a package from a local archive for testing.
  - `zoi package doctor`: Lint and validate `.pkg.lua` definitions before testing/building.
  - `zoi package inspect`: Parse a package definition and output resolved metadata (supports JSON).
  - `zoi package verify`: Verify integrity. With no arguments it checks every installed package's files against the digests recorded at install time; with `verify archive --file <PATH>` it checks a `.zpa` against its internal pooled manifest.
  - `zoi package delta`: Generate a `.zdelta` patch between an older and a newer `.zpa`, transferring only changed content as bsdiff patches.
  - `zoi package init-lsp`: Initialize LSP support with EmmyLua type definitions and `.luarc.json`.
...
- **Powerful CLI Tools:** Zoi offers a rich set of commands for package management and inspection:
  ...
  - `zoi download`: Download a package archive (.zpa) or source bundle (.zsa) to local cache or output directory.
  - `zoi cache`: Manage and populate the local package archive cache. Use `zoi cache clear` to free disk space by wiping the entire cache.
  - `zoi clone`: Clone the git repository of a package to a local directory.
  ...
  - `zoi migrate scoop`: Convert a Scoop manifest (`.json`) into a Zoi `.pkg.lua`.
  ...
  - `zoi rollback`: Revert a package to its previously installed version.
  - `zoi transaction undo`: Revert every operation recorded in a transaction (defaults to the most recent).
  ...
  - `zoi transaction`: Inspect recorded transactions and the files they changed.
  ...
  - `zoi doctor`: Checks for common issues with your Zoi installation and provides actionable suggestions. It performs the following checks:
    ...
    - **Registry Drift:** Detects if local package files (`.pkg.lua`) differ from the hashes recorded in the registry database.
    - **Lockfile Mismatch:** Warns if the current `zoi.lock` targets a different platform than the host.
...
- **Zoi Mini (Lightweight Service):** A minimal version of Zoi that operates without a local database, resolving package metadata and vulnerabilities on the fly from the Zoidberg registry. Perfect for one-liner installations via `zm.sh` or `zm.ps1`. Supports `install`, `update`, `uninstall`, and `list` commands with aliases (`i`, `up`, `un`, `ls`).
...
- `--pkg-dir <DIR>`: Additional directory to search for `.zpa` archives.
...
- `-a`, `--all`: List all security advisories in the database, not just for installed packages.
- `--registry <HANDLE>`: Filter advisories by specific registry handle.
- `--repo <REPO>`: Filter advisories by specific repository.
...
- `add <FILES...>`: Add `.zpa` archives to the local cache.
- `ls`: List all archives currently in the cache.
- `mirror add <URL>`: Add a binary/cache mirror base URL.
- `mirror remove <URL>`: Remove a configured binary/cache mirror.
- `mirror ls`: List configured binary/cache mirrors.
- `clear`: Clear the entire Zoi cache, including downloaded archives and cached package definitions.
...
- `<package>`: Package name, `@repo/name`, local `.pkg.lua` path, or URL.
- `[location]`: The directory to clone the repository into.
...
Enter a development shell for the current project. This command reads `zoi.lua`, ensures all packages in `pkgs` are installed, and sets up a subshell with `PATH`, `LD_LIBRARY_PATH`, `CPATH`, and `PKG_CONFIG_PATH` pointing to the project's dependencies.
...
- `[ARGS]...`: Arguments to pass to the executed binary.

**Options:**

- `--bin <NAME>`: Specific binary to run (required if the package provides multiple binaries).
- `-v`, `--verbose`: Show execution details (resolution, installation, sandboxing).
...
- `--verify`: Verify audit log chain integrity instead of printing entries.
- `--export <PATH>`: Export audit history to a file.
- `--ndjson`: Export as newline-delimited JSON instead of a JSON array. Requires `--export`.
...
- `validate <source>`: Validate a `.pkg.lua` or Zoi spec file for correctness.
...
If a requested package is already installed in the target scope at the same version and revision, Zoi skips it with an "already installed" message instead of reinstalling.
If an older version is installed, Zoi skips it and prints a hint pointing to `zoi update <package>`.
If the package is installed in a different scope than requested, Zoi notes that but proceeds installing into the requested scope. `--force` bypasses these checks entirely and reinstalls.
...
- `--purl <PURL>`: Install a package by its Package URL (PURL), e.g. `pkg:zoi/zoidberg/core/curl`.
- `--repo <REPO>`: Install from a git repository (e.g. 'zillowe/hello', 'gh:zillowe/hello').
...
- `--save`: Save the package to the project's `zoi.lua`.
- `--deps-only`: Install only the dependencies of the given packages, not the packages themselves.
- `--build-deps-only`: Install only the build dependencies declared by the given packages. Works for any source type (local `.pkg.lua`, registry package, URL, or git).
...
- `--frozen`: Install exactly what is pinned in `zoi.lock` (project scope only, lockfile must already exist, no lockfile updates are allowed, and recorded sub-package/dependency selections are preserved).
- `--retry <N>`: Retry failed downloads up to `N` attempts (minimum `1`, default `3`).
- `-v`, `--verbose`: Show additional install details, including package origins and preflight information.
- `--explain`: Print a dependency-selection and install-decision explanation, including why each package was chosen and which action will be taken.
...
- `-o`, `--outdated`: List only installed packages that have updates available in the registry.
...
Shows a package's manual in your system's `man` viewer (falling back to `info`).
...
- `--page <PAGE>`: Show a specific manual page (e.g. `zbsdiff.3`). Defaults to the page matching the package name in section 1.
...
- `--as-explicit` (alias: `--asexpl`): Mark packages as explicitly installed.
...
- `-o`, `--output <PATH>`: Output path for generated `.pkg.lua`.
- `--repo <REPO>`: Value for `metadata.repo` in generated package.
- `--name <NAME>`: Override output package name.
- `--version <VERSION>`: Override package version.
- `--maintainer-name <NAME>`: Maintainer name to set in metadata.
- `--maintainer-email <EMAIL>`: Maintainer email to set in metadata.
...
- `build <package_file>`: Build a package from a `.pkg.lua` file.
  - By default, Zoi uses a two-stage build: it first bundles the package into a source archive (`.zsa`) in a temporary location, then builds the distributable archive (`.zpa`) from that bundle. The intermediate `.zsa` is discarded after the build; use `zoi package bundle --sign` if you need a signed, distributable `.zsa`.
  - `--type <TYPE>`: The type of package to build (e.g. `source`, `pre-compiled`). If omitted, Zoi chooses `pre-compiled`, then `source`, then the first listed type.
  - `-p`, `--platform <PLATFORM>`: The platform to build for (default: `current`).
  - `-o`, `--output-dir <DIR>`: Directory to output the built package to.
  - `--no-zsa`: Skip creating the intermediate source bundle and build directly from the `.pkg.lua` file.
  - `--sign <KEY>`: Sign the package with the given PGP key.
  - `--sign-mode <embed|file>`: How to attach the signature when `--sign` is used (default: `embed`). `embed` stores a `manifest.sig` entry inside the archive so authenticity travels with it; `file` writes a legacy `<archive>.sig` sidecar.
  ...
  - `--version-override <VERSION>`: Override the package version.
  - `--method <native|docker>`: The build method to use (default: `native`).
  - `--image <IMAGE>`: The Docker image to use for the build (required for `--method docker`).
...
- `test <package_file>`: Run tests for a package. This command accepts the same arguments as `build`.
  - `--type <TYPE>`: The build type to use for testing. If omitted, Zoi chooses `pre-compiled`, then `source`, then the first listed type.
  - `-p`, `--platform <PLATFORM>`: The platform to test on.
  - `--sub <SUB_PACKAGES...>`: For a split package, test only the specified sub-packages.
  - `--version-override <VERSION>`: Override the package version for testing.
  - `--fakeroot`: Use fakeroot for the test build.
...
- `doctor <package_file>`: Lint and validate a `.pkg.lua` package definition. Performs consistency checks, including verifying that the metadata `repo` field matches the file's location in the registry.
  - `--platform <PLATFORM>`: Validate as a specific platform (defaults to current).
  - `--version-override <VERSION>`: Override package version while validating.
- `inspect <package_file>`: Parse a package definition and output its fully resolved metadata.
  - `--json`: Output as pretty-printed JSON (ideal for CI/CD and automation).
  - `--platform <PLATFORM>`: Resolve metadata for a specific platform.
  - `--version-override <VERSION>`: Override package version while inspecting.
- `init-lsp`: Initialize LSP support with EmmyLua type definitions and `.luarc.json`.
- `verify [installed|archive]`: Verify package integrity. Without arguments, checks every installed package's files against digests recorded at install time. With `archive --file <PATH>`, checks a `.zpa` archive against its internal pooled manifest.
  - `--scope <user|system>`: Scope to verify (installed mode).
  - `--archive <PATH>`: Path to the `.zpa` archive to verify (archive mode).
  - `--json`: Output results as JSON.
- `delta <old_archive> <new_archive>`: Generate a `.zdelta` patch between an older and a newer `.zpa` archive. The delta contains only the changed content as bsdiff patches, transferred through a zstd-compressed tar container. Also generates `.zdelta.hash`, `.zdelta.size`, and optionally `.zdelta.sig` sidecar files.
  - `-o, --output <PATH>`: Output path for the `.zdelta` file (defaults to `<new_archive>.zdelta`).
  - `--sign <KEY>`: Sign the delta patch with a PGP key from your Zoi keyring. Generates a `.zdelta.sig` file.
...
To revert an entire transaction, use `zoi transaction undo` instead.
...
- `--sort <name|repo|type>`: Sort results by field (default: `name`).
...
- `--hook`: Generate a shell hook script to be evaluated by your shell configuration (e.g. `eval "$(zoi shell --hook bash)"`).
...
- `--purl`: Display the Package URL (PURL) for the package.
...
- `-v`, `--verbose`: Show detailed git output and intermediate sync steps.
- `--fallback`: Fallback to other mirrors if the default one fails.
- `--no-pm`: Do not check for installed native package managers.
- `--force`: Rebuild all registry databases from scratch by removing existing caches, git clones, and re-cloning. Installed packages and their records are preserved.
...
- `set <handle-or-url>`: Set the default registry. Handles resolve against built-in registries; `default` sets the built-in `set: true` registry.
...
- `<action>`: `status`, `enable`, `disable`, or `crash <list|show|send|clear>` for local crash reports.
...
- `--save`: Remove the package from the project's `zoi.lua`.
- `-s`, `--recursive`: Recursively remove dependencies that are no longer needed.
- `--explain`: Show uninstall reasoning and reverse-dependency impact.
...
- `--interactive`: Interactively choose a subset of upgrade candidates (requires `--all`).
- `--explain`: Explain why each package is selected or skipped.
...
- `-g`, `--global`: Add to global configuration (user `versions` map) instead of the local project (`zoi.lua`).
- `-y`, `--yes`: Automatically answer yes to prompts (including JIT installation prompts).
...
- `list` (alias: `ls`): List available transaction logs with their start time and operation count.
- `show <id>`: Show the ordered operations recorded in a transaction.
- `files <id>`: List the unique files modified by a transaction.
- `undo [id]`: Revert all operations recorded in a transaction (installs are uninstalled, uninstalls are restored, upgrades are downgraded). Defaults to the most recent transaction when no ID is given (aliases: `rollback`).
