...
- **Lua-Based Project Configuration (`zoi.lua`):** Define project environments with a powerful, scriptable Lua API. Supports dynamic dependency selection, registry overrides, and complex environment setup.
...
- **Pinned Registry Inputs:** Lock registries to exact Git revisions (branch, tag, or commit hash) in `zoi.lua`. `zoi.lock` automatically pins these to SHA-1 hashes for absolute reproducibility.

- **Reproducible Project Installs (`--frozen`):** Rebuild a project environment from `zoi.lock` without re-resolving versions, sub-packages, or recorded dependency choices. Zoi Specification v2 lockfiles include directory hashes to verify the integrity of the entire local state.
