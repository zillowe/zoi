...
- `metadata{...}`: **(Required)** This function defines all the essential information about your package. It takes a Lua table as an argument. See the [Metadata Fields](#metadata-fields) section for all available options.
...
| `repo`         | `string` | The repository where the package belongs.                          |
| `version`      | `string` | The package version.                                               |
| `description`  | `string` | A short description of the package.                                |
| `website`      | `string` | The official website of the package.                               |
| `git`          | `string` | The git repository URL.                                            |
| `man`          | `string` | A URL to the package's manual.                                     |
| `maintainer`   | `table`  | A table with `name`, `email`, and `website` fields.                |
| `author`       | `table`  | A table with `name`, `email`, and `website` fields.                |
| `license`      | `string` | The SPDX license identifier.                                       |
| `bins`         | `list`   | A list of binaries that the package provides.                      |
| `conflicts`    | `list`   | A list of packages that this package conflicts with.               |
| `types`        | `list`   | A list of supported build types (e.g. `source`, `pre-compiled`).   |
| `sub_packages` | `list`   | A list of sub-packages for a split package.                        |
| `main_subs`    | `list`   | A list of default sub-packages to install for a split package.     |
| `updates`      | `list`   | A list of update information tables.                               |
| `hooks`        | `table`  | A table of lifecycle hooks.                                        |
| `package_type` | `string` | The type of package (`package`, `collection`, `app`, `extension`). |

## Split Packages

Zoi supports "split packages", which allow a single `.pkg.lua` file to produce multiple, smaller packages. This is useful for large projects like the Linux kernel, where you might want to install the kernel, headers, and documentation as separate packages.

To create a split package, you use the `sub_packages` and `main_subs` fields in your `metadata`.

- `sub_packages`: A list of all the sub-packages that can be built from this source.
- `main_subs`: A list of the sub-packages that should be installed by default when a user installs the base package name.
...
### Lifecycle Functions with Split Packages

When working with split packages, the `prepare`, `package`, `verify`, and `uninstall` functions receive a table as an argument containing the name of the sub-package being processed.
...
### The `SUBPKG` Global Variable

Inside your `.pkg.lua`, you can also use the `SUBPKG` global string variable to get the name of the sub-package currently being processed. This is useful for conditional logic outside of the main lifecycle functions.

### Staging Files with `zcp`

When building a split package, `zcp` will automatically stage files into a sub-package-specific directory within the final archive. This ensures that when a user installs a specific sub-package, only the correct files are extracted.
...
- `zrm(path)`: A function for the `uninstall()` step to remove files. The path can contain variables like `${pkgstore}`, `${usrhome}`, and `${usrroot}`.
...
If your package creates files outside of its installation directory (e.g. config files in the user's home directory), you can define an `uninstall` function to clean them up.

```lua
function uninstall()
  -- zrm can use variables to target specific locations
  zrm("${usrhome}/.config/my-app")
  zrm("${usrroot}/etc/my-app.conf")
end
```

#### Target Paths for `zrm`

The `path` for `zrm` uses special variables to target common locations:

- `${pkgstore}`: The package's versioned installation directory inside the Zoi store.
- `${usrhome}`: The current user's home directory (`~`).
- `${usrroot}`: The system's root directory (`/`). **Use with extreme caution.**
...
- `--type <TYPE>`: Specify the build type, e.g. `source` or `pre-compiled`. This value is available in your script as the `BUILD_TYPE` global variable.
- `--sub <SUB_PACKAGE>`: For a split package, build only the specified sub-package(s).
...
- `--sub <SUB_PACKAGE>`: For a split package, install only the specified sub-package(s) from the archive.
