...
- **`${pkgstore}` is treated as `/usr`**: For system-scoped packages, ZoiOS mirrors the `pkgstore` contents into the global `/usr` directory of the active generation.
  - Stage binaries to `${pkgstore}/bin/`.
  - Stage libraries to `${pkgstore}/lib/`.
  - Stage headers to `${pkgstore}/include/`.
- **Use `${usrroot}` for outside of `/usr`**: If a file *must* exist outside of the read-only `/usr` tree (like in `/etc` or `/var`), use `${usrroot}`.
  - **Important:** Files placed via `${usrroot}` are NOT part of the Generation symlink farm and will persist across system rollbacks. Use this only for persistent configuration or state.

```lua
function package()
    -- Available in the versioned Generation view (/usr/bin/mytool)
