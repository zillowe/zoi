...
install({
  ...
  },
  {
    ...
    platforms = { "all" },
    build_commands = { "make" },
    install_commands = { 'make install DESTDIR="${Zoi_DESTDIR}"' }
  }
})
```
...
1.  **`zoi package meta <path/to/file.pkg.lua>`**:
    Generates a `*.meta.json` file. This file resolves all the dynamic Lua script logic for each platform into a static description of all assets, checksums, and signatures.
