...
| `package_type` | `string` | The type of package. Can be `package` (default), `collection`, `app`, or `extension`. See [Package Types](#package-types).        |
| `sub_packages` | `list`   | A list of sub-package names to define a split package. See [Split Packages](#split-packages).                                     |
...
| `hooks`        | `table`  | A table defining scripts to run at different points in the package lifecycle. See [Advanced Features](#advanced-features).        |
...
| `zcp(source, dest)`                       | Copies files/directories from `BUILD_DIR` to the `STAGING_DIR`. See [Staging Destinations](#staging-destinations-for-zcp). |
...
The `destination` argument for the `zcp` function uses special variables to place files in the correct location within the final package archive.
...
If your package has multiple `types` (e.g. `source`, `pre-compiled`), Zoi will automatically select one. You can force a specific type with the `--type` flag:

```sh
zoi install --type source /path/to/your-package.pkg.lua
```

### Step 2: Build the Distributable Archive

Once your package installs correctly locally, build the final `.pkg.tar.zst` archive. This is the file that would be distributed via a package registry.
...
## 8. Full Walkthrough: 'hello' Package
