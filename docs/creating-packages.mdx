...
# (Optional) A URL to a markdown manual for `zoi man`.
man: https://example.com/my-cli/MAN.md
...
- `man`: A URL to a markdown manual file that can be viewed with `zoi man`.

### Package Types

Zoi supports different types of packages. You can specify the type using the `type` field. This guide will cover how to create these different types in a later step.
...
- `app`: An application template used with `zoi create`.
- `extension`: A package that can modify Zoi's configuration.
- `library`: A software library.
- `script`: A package that runs a set of inline shell commands.
...
installation:
  - type: source
    ...
    commands:
      - "make build"
      - "mv ./bin/compiler {store}/{name}" # {store} is the path to Zoi's install directory
```
...
Both `build` and `runtime` dependency groups can be defined as a simple list of required dependencies, or as a more advanced object with `required`, `optional`, and selectable `options`.
...
## Step 5: Creating Specific Package Types

Beyond a standard `package`, Zoi supports several special package types for different use cases. You define the type of your package using the `type` field in your `pkg.yaml`.
...
For a more in-depth guide on extensions, see the [Extensions documentation](/docs/zds/zoi/extensions).
...
A `script` package is a special package type that doesn't install a binary but instead runs a series of inline shell commands. This is useful for performing custom setup actions, orchestrating other tools, or for installers that consist of a series of commands rather than a single remote script.

This is distinct from the `script` **installation method**, which is used for packages that download and run a single remote script file (like `install.sh`).

To create a `script` package, set the `type` to `script` and define a top-level `script` block with `install` and optional `uninstall` commands.

```yaml
# my-setup-script.pkg.yaml
...
script:
  - platforms: ["linux", "macos"]
    # These commands run when the user installs the package.
    install:
      - "echo 'Creating configuration directory...'"
      - "mkdir -p ~/.config/my-app"
      - "echo 'hello: world' > ~/.config/my-app/config.yml"
    # These commands run when the user uninstalls the package.
    uninstall:
  ...
  - platforms: ["windows"]
    install:
      - "echo 'Creating configuration directory...'"
      - "mkdir -p $env:APPDATA/my-app"
      - "echo 'hello: world' | Out-File -FilePath $env:APPDATA/my-app/config.yml"
    uninstall:
      - "echo 'Removing configuration...'"
      - "rm -r $env:APPDATA/my-app"
```

When a user runs `zoi install my-setup-script`, Zoi will ask for confirmation and then execute the commands listed under `install` for the user's platform. The `uninstall` commands are run with `zoi uninstall`.

## Step 6: Advanced `pkg.yaml` Features
...
Once your package is tested and ready, the final step is to publish it. This can involve submitting it to the official Zoi repositories or hosting it in your own private or public repository.

For a complete guide on publishing, please see:

- **[Publishing Packages](/docs/zds/zoi/publishing-packages)**

Once published to the official repositories, your package will be discoverable on the [Nel Registry](https://nel.zillowe.qzz.io).
