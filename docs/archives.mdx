...
Zoi supports installing compressed binary releases via the `com_binary` method in `.pkg.lua`. The following archive formats are supported across install, exec, and upgrade paths:
...
- You can override this by specifying `bin_path` in the `com_binary` installation method. This can be a relative path (e.g. `bin/tool`) or just a filename (e.g. `tool.exe`). On Windows targets, if your `bin_path` does not end with `.exe`, Zoi will also look for the same path with `.exe` appended.
