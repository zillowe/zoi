...
The standard Rust build process. The release profile is optimized for size and performance (see `Cargo.toml`).
...
This produces the `zoi`, `zoi-mini`, and `zoid` binaries in `target/release/`.

### Using the Justfile

The provided `Justfile` simplifies the build and installation process.

```sh
# Configure installation paths (creates config.just)
...
docker build \
  ...
  --build-arg ZOI_ABOUT_PACKAGER_AUTHOR="Your Team" \
  --build-arg ZOI_ABOUT_PACKAGER_EMAIL="packages@example.com" \
  --build-arg ZOI_ABOUT_PACKAGER_HOMEPAGE="https://example.com" \
  -t zoi .
```
...
Several aspects of Zoi can be configured at build time by setting environment variables. These are processed by `build scripts`.
...
1. Place your public key files (`.asc`) in the `crates/core/src/builtin/pgp/` directory.
2. Build Zoi as usual.

The build system (`build scripts`) will automatically detect these files and embed them as static assets. When the resulting binary is run, it will ensure these keys are present in the user's local Zoi keyring.
