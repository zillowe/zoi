...
Dependencies are specified in the `dependencies` section. Both `runtime`, `build`, and `test` dependency groups can be defined as a simple list of required dependencies, or as a more advanced object with `required`, `optional`, and selectable `options`.

### Dependency String Format

Each entry follows the format `manager:package[@version][:description]`. If the manager prefix is omitted, Zoi treats the dependency as a `zoi` dependency.
...
- **Split packages:** Use the dependency group's `sub_packages` map to add dependencies that only apply to a specific sub-package. Dependency strings themselves do not have a separate sub-package field.
- **Version:** Zoi dependencies support standard SemVer requirements such as `=1.2.3`, `>=2.0.0`, `^1.2`, and `~1.2.3`. External dependencies support exact versions and safe version forms beginning with `=`, `^`, or `~`; their package manager ultimately decides whether a version can be enforced. An optional `v` prefix is supported (e.g. `@v1.2.3`).
  - Examples: `npm:typescript@5.3.2`, `cargo:bat@v0.24.0`, `apt:curl=7.68.0-1ubuntu2.18`.
  - For `zoi` packages, you can use a channel name like `zoi:my-pkg@stable`.
  - External dependency names and versions must not contain shell-control syntax. Zoi rejects these values before invoking the external package manager.
...
### Local `.pkg.lua` Dependencies

Zoi dependencies can point to another package definition relative to the declaring `.pkg.lua` file:

```lua
dependencies({
  runtime = {
    "zoi:./shared/helper.pkg.lua",
    "zoi:../common/utility"
  }
})
```

A directory is accepted when it contains `directory-name.pkg.lua` or exactly one `.pkg.lua` file. Relative paths are resolved independently of the caller's current working directory, and the selected file and version are recorded in `zoi.lock`.
...
dependencies({
  ...
  test = {
    -- Test dependencies installed only when running `zoi package test` or `zoi package build --test`
    required = { "npm:jest", "cargo:cargo-nextest" }
  }
})
...
-- Example of per-type build dependencies as a list (the form used by
-- published registry indexes)
dependencies({
  build = {
    { type = "source", packages = { "native:gcc", "native:make" } },
    { type = "pre-compiled", packages = { "native:unzip" } },
  }
})
```
...
In `.pkg.lua` files, you can use Lua's string concatenation and the global `PKG` table to create dynamic version dependencies. The `PKG` table contains the fields defined in your `metadata{}` block.
...
| `go`                  | ✔️ Yes          | Uses `go install <module>@<version>` when a version is supplied.                                                                                                       |
...
Zoi includes intelligence to avoid unnecessary failures when resolving dependencies from external managers:

- **Skip if Missing**: If a dependency specifies an OS-specific manager (like `apt` or `pacman`) that is not available on your current system, Zoi will print a warning and skip that dependency instead of failing the installation. This allows for creating cross-platform `.pkg.lua` files that list requirements for multiple distributions.
- **Already Installed Check**: Zoi checks if a package is already installed via its respective manager before attempting to run an install command. This speeds up builds and avoids noise from managers like `cargo` or `npm`.
...
| `zig`            | Zig                  | `zig:https://github.com/user/repo/archive/main.tar.gz` |
...
- Some managers (e.g. `go`, `jsr`, `volta`, `uv`, `zig`) do not provide reliable uninstall; Zoi prints a notice and skips.
