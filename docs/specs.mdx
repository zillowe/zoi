...
The `zoi.lock` file records enough information to reproduce a project environment, including its evaluated configuration, imported repositories, registries, root requirements, package sources, selected dependency features, and installed content.
...
```json
{
  ...
  "installed_packages": {
    "@core/curl": {
      ...
      "resolved_dependencies": ["openssl@3.5.0", "zlib@1.3.1"],
      "chosen_options": [],
      "chosen_optionals": [],
      "git_sha": null,
      "required_by": []
...
- **`version`**: Lockfile format version, currently `"2"`.
- **`platform`**: Platform for which this lock state was generated.
- **`manifest`**: Project manifest filename and SHA-512 hash.
- **`project`**: Hashes for the manifest, tasks, environments, shell configuration, and checks, plus project-local mode.
- **`registries`**: Registry URL and exact Git commit.
- **`imports`**: Import alias to requested repository, resolved commit, manifest hash, optional manifest path, and resolved exports.
- **`root_requirements`**: Original root sources and normalized `PackageSpec` values.
- **`installed_packages`**: Fully resolved package state keyed by package identifier.
- **`packages_hash`**: SHA-512 hash of the project store.
- **`registries_hash`**: SHA-512 hash of the project registry database.

### Package fields

- Identity and resolution: `name`, `sub_package`, `repo`, `repo_type`, `version`, `epoch`, `revision`, `registry`, and `platform`.
- Installation: `why`, `type`, `install_method`, `installed_sub_packages`, and content `hash`.
- Provenance: `definition_hash` and `source`.
  - `source.kind`: `registry`, `local`, `url`, `git`, or `unknown`.
  - `source.request`: Original source request.
  - `source.url`, `source.revision`, and `source.path`: Resolved source details when applicable.
- Reproduction: declared `dependencies`, exact `resolved_dependencies`, `chosen_options`, `chosen_optionals`, `git_sha`, and `required_by`.

`--frozen` validates the lock format and platform, reconstructs pinned registry and import revisions, and uses the recorded source and package state instead of re-resolving the mutable `zoi.lua` graph.
